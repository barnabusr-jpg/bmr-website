...
import { 
  Activity, Building2, ChevronUp, ChevronDown, 
  Shield, Zap, Binary, ZoomIn, Hammer, Mail, FileText, CheckCircle, Clock 
} from "lucide-react";
import { supabase } from "@/lib/supabaseClient";

// 🏛️ 2.0 PROPRIETARY VERBIAGE SUITE
const BMR_IP_SUITE = {
  directives: [
    { 
      id: "DIR_01", 
...
export default function AdminDashboard() {
  ...
  }, [fetchLedger, expandedRow, refreshActiveNodes]);

  return (
    <div className="min-h-screen bg-[#020617] text-slate-200 font-sans tracking-tighter text-left selection:bg-red-600/30">
      {/* 🧭 2.0 FORENSIC NAV */}
      <nav className="fixed top-0 left-0 right-0 h-24 bg-black/90 backdrop-blur-md border-b border-slate-900 z-50 px-10 flex items-center justify-between">
        <div className="flex items-center gap-8">
          ...
          <div className="flex gap-1 bg-slate-900 p-1">
            <button onClick={() => setActiveTab('ledger')} className={`px-6 py-2 text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === 'ledger' ? 'bg-red-600 text-white shadow-lg' : 'text-slate-500 hover:text-white'}`}>Live_Ledger</button>
      ...
      <main className="pt-40 px-10 max-w-[1600px] mx-auto pb-32">
        <AnimatePresence mode="wait">
          {activeTab === 'ledger' ? (
            <motion.div key="ledger" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} className="space-y-4">
              
              {/* 📋 HEADER DIRECTIVES */}
              <div className="grid grid-cols-12 px-10 mb-6 opacity-30 font-mono text-[9px] font-black uppercase tracking-[0.4em] italic">
                  ...
                  <div onClick={() => toggleRow(audit.id)} className="grid grid-cols-12 items-center p-8 cursor-pointer group">
                    <div className="col-span-6 flex items-center gap-6">
                      ...
                      <div>
                        ...
                        <div className="text-[10px] text-slate-600 font-mono mt-2 uppercase tracking-widest italic">{audit.lead_email}</div>
                      </div>
                    </div>
                    ...
                    <div className="p-10 pt-0 border-t border-slate-900/50">
                          ...
                          return (
                            <div key={role} className="border-2 border-slate-900 p-8 bg-slate-950/40 relative min-h-[160px] flex flex-col justify-between">
                              <div className="flex justify-between items-start">
                                ...
                                {isDone ? <CheckCircle className="text-red-600" size={18}/> : <Clock className="text-slate-900" size={18}/>}
                              </div>
                              <div className={`text-4xl font-black italic uppercase tracking-tighter ${isDone ? 'text-white' : 'text-slate-900'}`}>
                      ...
                      <div className="flex justify-between items-center border-t border-slate-900/50 pt-10">
                        <div className="flex gap-4">
                          <button className="bg-red-600/80 text-white px-8 py-4 font-black uppercase italic text-[11px] tracking-widest hover:bg-red-600 flex items-center gap-3">
                            <Mail size={18} /> RE-DISPATCH_PROTOCOL
                          </button>
                          <button className="bg-[#b48c3b] text-black px-8 py-4 font-black uppercase italic text-[11px] tracking-widest hover:bg-yellow-500 flex items-center gap-3">
                            <Zap size={18} /> FORCE_SYNTHESIS
                          </button>
                        </div>
                        <button onClick={() => window.open(`/results/${audit.id}`)} className="bg-white text-black px-12 py-4 font-black uppercase italic text-[11px] tracking-widest hover:bg-red-600 hover:text-white transition-all flex items-center gap-3">
                          <FileText size={20} /> GENERATE_FORENSIC_DOSSIER
                        </button>
                      </div>
                    </div>
                  )}
            ...
            <motion.div key="frameworks" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="space-y-20">
              {/* 🧪 2.0 SERVICE TIERS (Verbiage Weaving) */}
              <section>
              ...
              <section>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  {BMR_IP_SUITE.directives.map((d) => (
                    <div key={d.id} className="p-12 border-2 border-slate-900 bg-slate-950 hover:border-red-600 transition-all group relative">
