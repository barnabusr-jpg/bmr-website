            ...
            <button 
              onClick={(e) => {
                ...
                if (activeAudit) {
                  ...
                  // Cleanly format and match sector configurations
                  const targetSector = (activeAudit.sector || "SERVICES").toUpperCase().trim();
                  ...
                  const userNode = nodeDetails.find(n => n.persona_type?.toUpperCase() === 'USER');

                  // Route smoothly directly to triage panels
                  window.open(
                    `/forensic?entity_code=${encodeURIComponent(entityCode)}&sector=${encodeURIComponent(targetSector)}&pillar=${targetPillar}&exec=${encodeURIComponent(execNode?.email || "")}&tech_mgmt=${encodeURIComponent(techNode?.email || "")}&ops_mgmt=${encodeURIComponent(mgrNode?.email || "")}&sys_user=${encodeURIComponent(userNode?.email || "")}&auth=admin_verified_secure`, 
                    '_blank'
                  );
                } else {
            ...
            <motion.div key="ledger" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }} className="space-y-6">
              ...
              {data.length === 0 ? (
                ...
                data.map((audit) => {
                  ...
                  // 🧠 SPLIT INDIVIDUAL PROFILE COEFFICIENTS (1:1 STRATEGY INTAKE MATCH)
                  ...
                  } else if (sectorKey === 'industrial' || sectorKey === 'operations' || sectorKey === 'manufacturing') {
                    sectorInflationMultiplier = 1.5; 
                          ...
                          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 mb-8">
                            <div className="lg:col-span-5 border border-slate-900 bg-slate-950 p-6 space-y-4 font-mono">
                              ...
                              <div className="space-y-3 pt-2 border-t border-slate-900 text-xs">
                                ...
                                <div className="flex justify-between reset-border-t border-slate-900 pt-2 text-sm"><span className="text-slate-400 font-black">TOTAL EXPENSE LEAKAGE:</span><span className="text-red-600 font-black">${totalLeakage.toLocaleString(undefined, {maximumFractionDigits:0})}</span></div>
                              </div>
                            </div>
