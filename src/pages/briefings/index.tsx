...
export default function EvidenceVault() {
  ...
  return (
    <div className="min-h-screen bg-[#020617] text-white font-sans italic selection:bg-red-600/30 uppercase font-black overflow-x-hidden">
      ...
      <main className="pt-40 pb-20 px-6 max-w-[1600px] mx-auto text-left italic">
        {/* --- HEADER SECTION --- */}
        ...
        {/* 🛡️ RESPONSIVE GRID HARDENING: 
            'grid-cols-1' ensures stacking on mobile.
            'lg:grid-cols-3' restores the 3-column view on desktop.
        */}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8 font-black italic">
          {caseStudies.map((item) => (
            <Link key={item.slug} href={`/briefings/case-study/${item.slug}`} className="group no-underline block h-full">
              <div className="bg-slate-950 border-2 border-slate-900 p-8 lg:p-12 relative overflow-hidden hover:border-red-600 transition-all shadow-2xl h-full min-h-[500px] md:min-h-[550px] flex flex-col justify-between italic">
                ...
                <div className="relative z-10 w-full italic">
                  <div className="font-mono text-[10px] text-red-600 font-black tracking-widest uppercase italic">
                    FILE_REF: {item.id} // <span className="text-slate-500 italic">{item.node}_NODE</span>
                  </div>
                  
                  {/* 🛡️ TITLE SNAPPING: 'break-words' and 'leading-[0.9]' keeps the text contained */}
                  <h2 className="text-4xl lg:text-[40px] xl:text-[48px] font-black uppercase tracking-tight text-white mt-10 mb-8 leading-[0.9] break-words italic">
                    {item.title}
                  </h2>
                  
                  <p className="text-base lg:text-lg text-slate-500 border-l-2 border-slate-800 pl-6 font-black italic uppercase leading-tight normal-case italic">
                    "{item.summary}"
                  </p>
                </div>
