...
export default function EvidenceVault() {
  const caseStudies = [
    { id: "CASE_01", title: "EXECUTIVE VARIANCE", slug: "chatbot-liability", node: "EXECUTIVE" },
    { id: "CASE_02", title: "FORENSIC INSIGHTS", slug: "salesforce-failure", node: "TECHNICAL" },
    { id: "CASE_03", title: "LOGIC SHEAR", slug: "lyft-logic-shear", node: "MANAGERIAL" }
  ];
        ...
        <div className="flex flex-col md:flex-row justify-between items-end mb-24 border-b-2 border-slate-900 pb-12">
          <div className="border-l-4 border-red-600 pl-8">
            <h1 className="text-7xl md:text-9xl font-black uppercase italic tracking-tighter leading-none">
              EVIDENCE <br /><span className="text-red-600 italic text-8xl md:text-[140px]">VAULT.</span>
            </h1>
          ...
          <Lock className="text-slate-900 hidden md:block mb-4 opacity-30" size={120} />
        </div>

        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {caseStudies.map((item) => (
            <Link key={item.id} href={`/briefings/briefings/case-study/${item.slug}`} className="group no-underline">
              <div className="bg-slate-950 border-2 border-slate-900 p-12 relative overflow-hidden group hover:border-red-600 transition-all shadow-2xl min-h-[550px] flex flex-col justify-between">
                ...
                <div className="relative z-10">
                  <span className="font-mono text-[10px] text-red-600 font-black tracking-[0.4em] uppercase italic">
                  ...
                  <p className="text-lg text-slate-500 font-medium italic leading-relaxed mb-12 border-l-2 border-slate-900 pl-6">
