                ...
                <div className="space-y-12">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
                    {[
                      ...
                      { id: "healthcare", label: "HEALTHCARE", risk: "LIABILITY" },
                      { id: "tech", label: "TECH", risk: "DEBT" },
                      { id: "services", label: "SERVICES", risk: "LABOR" }
                    ].map((s) => (
                      <button 
                        key={s.id} 
                        onClick={() => setSector(s.id as Sector)}
                        className={`p-4 border text-[9px] font-black uppercase tracking-widest transition-all ${sector === s.id ? 'bg-red-600 border-red-600 text-white shadow-[0_0_15px_rgba(220,38,38,0.4)]' : 'border-slate-800 text-slate-500 hover:border-slate-600'}`}
                      >
                        <div>{s.label}</div>
                        <div className="text-[7px] opacity-40 font-mono mt-1">{s.risk}</div>
                      </button>
                    ))}
                  </div>
