...
export default function AssessmentPage() {
  ...
  const questions: Record<string, Question[]> = {
    executive: [
      { id: 'exec_1', text: "What is your target labor reduction from AI (e.g., 20%, 30%, 50%)?", weight: "Labor Leakage", type: "range", min: 0, max: 100 },
    ...
    technical: [
      { id: 'tech_1', text: "For every 10 mins of AI output, how many minutes are spent fixing it?", weight: "Verify:Serve", type: "range", min: 0, max: 100 },
    ...
    shared: [
      ...
      { id: 'sh_4', text: "How confident are you that the AI data hasn't drifted in the last 30 days?", weight: "Drift Confidence", type: "range", min: 0, max: 100 },
    ]
  };
  ...
  const selectOption = (val: number | string) => {
    ...
    setTimeout(() => setStep(s => s + 1), 300); // Slight delay for UX
  };
  ...
  const handleFinalize = async () => {
    ...
    try {
      ...
      router.push('/pulse-check/results'); // Fallback routing
    }
  };
        ...
        <div className="max-w-2xl w-full space-y-12">
          ...
          <div className="space-y-4">
            <div className="flex justify-between items-center text-[10px] font-mono uppercase tracking-[0.3em]">
              ...
              <div className="text-slate-600 italic flex items-center gap-2"><Lock size={10} /> INTEGRITY PROTOCOL ACTIVE</div>
            </div>
          ...
          <AnimatePresence mode="wait">
            <motion.div key={step} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="min-h-[400px]">
              ...
              {step === 0 ? (
                ...
                <div className="text-center py-20 space-y-10 border border-red-900/20 p-12 bg-red-950/5">
