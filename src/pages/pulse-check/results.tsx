...
import { AlertTriangle, Zap, ShieldAlert, Activity, ArrowRight } from 'lucide-react';
...
export default function PulseCheckResults() {
  ...
  if (!result) return (
    <div className="min-h-screen bg-slate-950 flex items-center justify-center font-mono text-red-600 uppercase">
      <Activity className="animate-pulse mr-2" /> Deciphering Logic Fractures...
    </div>
  );

  const daysToCollapse = Math.round(10 / Math.abs(result.fractureVelocity) * 30);
        ...
        <div className="max-w-4xl mx-auto space-y-12">
          ...
          <div className="text-center py-16 border border-slate-900 bg-slate-900/20 relative">
            ...
            <div className="max-w-xl mx-auto px-6 space-y-8">
              <p className="text-slate-400 font-mono text-[10px] uppercase tracking-[0.2em] border-y border-slate-800 py-4">
              ...
              <div className="space-y-4 text-left">
                ...
                <div className="w-full bg-slate-900 h-4 border border-slate-800 p-0.5">
                  <motion.div 
                    initial={{ width: 0 }} 
                    animate={{ width: `${(result.deltaGap / 5) * 100}%` }} 
              ...
              {/* PROFIT HEMORRHAGE */}
              <div className="bg-red-600/10 border border-red-600/30 p-8">
            ...
            <div className="grid md:grid-cols-2 gap-4 mt-12 px-6">
              <button onClick={() => window.location.href='/audit'} className="bg-red-600 p-6 flex flex-col items-center group transition-all">
                ...
                <span className="font-black text-[10px] tracking-widest">RUN 30-QUESTION AUDIT</span>
              </button>
