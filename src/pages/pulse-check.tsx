...
// ... [LOCAL_QUESTIONS and sectors constants remain the same] ...
...
export default function PulseCheck() {
  ...
  const validateIntake = () => {
    return operatorName && entityName && email && email === confirmEmail && isBusinessEmail(email);
  };
    ...
    <div className="min-h-screen bg-[#020617] text-white selection:bg-red-600/30 font-sans italic overflow-x-hidden uppercase font-black">
      ...
      <main className="min-h-screen flex flex-col items-center justify-center py-40 px-6 relative text-center">
        ...
        <AnimatePresence mode="wait">
          {step === 'triage' && (
            <motion.div key="triage" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="w-full max-w-5xl space-y-16">
              <div className="border-b border-slate-900 pb-12 flex flex-col items-center">
                ...
                {/* 🔴 RESTORED: FULL LINE RED PULSE INDICATOR */}
                <motion.div 
          ...
          {step === 'audit' && (
            <motion.div key="audit" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="w-full max-w-5xl space-y-12">
              <div className="flex flex-col items-center gap-6 mb-16 border-b border-slate-900 pb-8">
                ...
                <motion.div 
                  ...
                  className="flex items-center gap-4 text-red-600 font-mono text-[11px] font-bold tracking-[0.3em] uppercase italic"
                >
                  <div className="w-2 h-2 bg-red-600 rounded-full shadow-[0_0_10px_rgba(220,38,38,1)]" />
                  <span>LOG: BMR 2026 0{currentDimension + 1} // FOCUS: {sector.toUpperCase()} {selectedLens}</span>
                </motion.div>
              </div>
              <h2 className="text-4xl md:text-7xl font-black italic uppercase leading-none min-h-[200px] tracking-tighter mx-auto max-w-4xl text-white">{LOCAL_QUESTIONS[currentDimension]?.text}</h2>
              <div className="grid grid-cols-1 gap-4 max-w-3xl mx-auto w-full">
                {LOCAL_QUESTIONS[currentDimension]?.options.map((opt, i) => (
                  <button key={i} className="py-8 px-10 border-2 border-slate-900 bg-slate-950/40 hover:border-red-600 transition-all text-center uppercase font-black text-2xl group italic text-slate-300 hover:text-white"
                    onClick={async () => {
      ...
      {/* 🛠️ ADMIN DASHBOARD: MOVED TO BOTTOM LEFT */}
      <footer className="py-10 border-t border-slate-900 flex justify-between items-center px-10">
        <div className="flex items-center gap-6">
          ...
          <p className="text-[9px] text-slate-800 tracking-widest font-mono italic">PRIVATE_PROTOCOL_V2.6</p>
        </div>
        <p className="text-[9px] text-slate-900 font-mono italic opacity-50">BMR SOLUTIONS // FAIRFAX COUNTY, VA</p>
      </footer>
    </div>
  );
}
