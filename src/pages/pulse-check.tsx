    ...
    <div className="min-h-screen bg-[#020617] text-white selection:bg-red-600/30 font-sans">
      ...
      <main className="pt-32 pb-16 md:pt-56 md:pb-32 w-full overflow-hidden px-6 md:px-10">
        <div className="max-w-[1280px] mx-auto flex flex-col lg:flex-row items-start justify-between gap-12 lg:gap-16 text-left">
          <div className="w-full lg:w-[55%] shrink-0 text-left">
            <div className="border-l-[6px] border-red-600 pl-6 md:pl-10 mb-10 md:mb-16">
              <h2 className="text-red-600 font-mono text-xs font-black tracking-[0.5em] uppercase italic mb-5">BMR_SOLUTIONS // AI_EFFICIENCY_AUDIT</h2>
              <h1 className="text-5xl sm:text-6xl md:text-[clamp(3.5rem,8vw,7rem)] font-black italic leading-[0.85] uppercase m-0 tracking-tighter">Initialize <br /><span className="text-red-600">Intake.</span></h1>
            </div>
            ...
            <div className="border-t border-[#1e293b] pt-10 mb-12">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                {['Executive', 'Managerial', 'Technical'].map((node) => (
                  <button key={node} onClick={() => setSelectedNode(node)} className={`py-8 px-6 border-2 font-black uppercase italic tracking-widest text-[10px] transition-all text-left ${selectedNode === node ? "border-red-600 bg-red-600/10 text-white" : "border-slate-900 bg-slate-950 text-slate-500 hover:border-slate-700 hover:text-white"}`}>
                    {node}_Node
                  </button>
                ))}
              </div>
            ...
            <div className="h-20">
              {selectedNode && (
                <button className="bg-white text-black py-6 px-12 font-black uppercase tracking-[0.3em] text-[10px] hover:bg-red-600 hover:text-white transition-all flex items-center gap-4 italic italic">
          ...
          <div className="w-full lg:w-[40%] mt-8 lg:mt-24 shrink-0">
            <div className="bg-[#0f172a]/40 border-2 border-[#1e293b] p-8 md:p-14 relative overflow-hidden">
              ...
              <p className="text-slate-400 text-xl md:text-2xl font-medium italic font-sans uppercase leading-tight italic">
