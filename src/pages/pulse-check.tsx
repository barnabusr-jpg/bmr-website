...
import { Banknote, Stethoscope, Factory, ShoppingCart, ChevronRight, ShieldAlert } from "lucide-react";
...
const auditQuestions = [
  { id: 1, text: "Are autonomous agents currently operating without human-in-the-loop overrides?", category: "OPERATIONAL" },
  { id: 2, text: "Is there a documented forensic trail for every AI-generated decision?", category: "LEGAL" },
  { id: 3, text: "Does the system have an automated 'Kill-Switch' for logic-shear events?", category: "SECURITY" },
  // Add remaining 12 queries here
];
    ...
    <div className="min-h-screen bg-[#020617] text-white selection:bg-red-600/30 font-sans italic overflow-x-hidden uppercase font-black relative flex flex-col">
      ...
      <main className="flex-grow flex flex-col items-center justify-center py-40 px-6 relative text-center">
        <AnimatePresence mode="wait">
          ...
          {step === 'triage' && (
            <motion.div key="triage" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="w-full max-w-5xl space-y-16">
              <div className="border-b border-slate-900 pb-12 flex flex-col items-center">
                ...
                <motion.div animate={{ opacity: [1, 0.4, 1] }} transition={{ repeat: Infinity, duration: 2 }} className="flex items-center gap-4 mt-8 text-red-600">
                   ...
                   <p className="text-[10px] font-mono uppercase tracking-[0.5em] font-bold">
                     {selectedLens ? `STATUS: FOCUS LOCKED [${selectedLens}]` : "STATUS: AWAITING FOCUS SELECTION"}
                   </p>
                </motion.div>
              </div>
            ...
            <motion.div key="intake" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="w-full max-w-4xl space-y-12 italic text-center">
              <div className="border-b border-slate-900 pb-10 flex flex-col items-center">
                ...
                <motion.div animate={{ opacity: [1, 0.4, 1] }} transition={{ repeat: Infinity, duration: 2 }} className="flex items-center gap-4 mt-8 text-red-600">
                   <div className="w-3 h-3 rounded-full bg-red-600 shadow-[0_0_15px_rgba(220,38,38,1)]" />
                   <p className="text-[10px] font-mono uppercase tracking-[0.5em] font-bold">
                     {validateIntake() ? "VALIDATION COMPLETE // INITIALIZE" : "STATUS: PROVIDE ENTITY DETAILS"}
                   </p>
                </motion.div>
              </div>

              <div className="bg-slate-950 border border-slate-900 p-8 md:p-12 space-y-12 shadow-2xl text-left">
          ...
          {/* STEP 3: AUDIT QUESTIONS (FIXES THE BLACK SCREEN) */}
          {step === 'audit' && (
            <motion.div key="audit" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="w-full max-w-4xl space-y-12 text-center">
              <div className="flex flex-col items-center space-y-6">
                <div className="flex items-center gap-4 text-red-600 font-mono text-[10px] tracking-[0.5em]">
                  <ShieldAlert size={16} />
                  FORENSIC_PROBE: {currentQuestion + 1} / {auditQuestions.length}
                </div>
                <h2 className="text-4xl md:text-6xl font-black uppercase italic tracking-tighter leading-tight">
                  {auditQuestions[currentQuestion].text}
                </h2>
              </div>

              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-12">
                {["AFFIRMATIVE", "NEGATIVE"].map((ans) => (
                  <button key={ans} onClick={handleAnswer}
                    className="p-12 border-2 border-slate-900 bg-slate-950 hover:border-red-600 transition-all group flex items-center justify-between">
                    <span className="text-3xl font-black italic uppercase tracking-tighter group-hover:text-red-600">{ans}</span>
                    <ChevronRight className="text-slate-800 group-hover:text-red-600" />
                  </button>
                ))}
              </div>
            </motion.div>
