...
export default function PulseCheck() {
  ...
  return (
    <div className="min-h-screen bg-[#020617] text-white selection:bg-red-600/30 font-sans italic overflow-x-hidden uppercase font-black">
      <Header />
      <main className="min-h-[100vh] flex flex-col items-center justify-center py-40 px-6 relative text-center">
        ...
        <AnimatePresence mode="wait">
            ...
            <motion.div key="triage" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="w-full max-w-5xl space-y-16">
              <div className="border-b border-slate-900 pb-12 flex flex-col items-center">
                ...
                <div className="flex items-center gap-4 mt-8">
                   <div className={`w-3 h-3 rounded-full animate-pulse transition-all duration-500 ${selectedLens ? 'bg-red-600 shadow-[0_0_15px_rgba(220,38,38,1)]' : 'bg-slate-800'}`} />
                   <p className={`text-[10px] font-mono uppercase tracking-[0.5em] font-bold ${selectedLens ? 'text-white' : 'text-slate-600'}`}>
                     {selectedLens ? `FOCUS: ${selectedLens} // SELECT SECTOR` : "STATUS: AWAITING FOCUS SELECTION"}
                   </p>
                </div>
              </div>
          ...
          {step === 'intake' && (
            <motion.div key="intake" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="w-full max-w-4xl space-y-12 italic">
              <div className="border-b border-slate-900 pb-10 flex flex-col items-center">
                ...
                <div className="flex items-center gap-4 mt-8">
                   <div className={`w-3 h-3 rounded-full animate-pulse transition-all duration-500 ${validateIntake() ? 'bg-red-600 shadow-[0_0_15px_rgba(220,38,38,1)]' : 'bg-slate-800'}`} />
                   <p className={`text-[10px] font-mono uppercase tracking-[0.5em] font-bold ${validateIntake() ? 'text-white' : 'text-slate-600'}`}>
          ...
          {step === 'audit' && (
            <motion.div key="audit" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="w-full max-w-5xl space-y-12">
              ...
              <h2 className="text-4xl md:text-7xl font-black italic uppercase leading-none min-h-[200px] tracking-tighter mx-auto max-w-4xl">{LOCAL_QUESTIONS[currentDimension]?.text}</h2>
