    ...
    <div className="min-h-screen bg-[#020617] text-white font-sans selection:bg-red-600/30">
      ...
      <main className="pt-44 pb-24 px-6 text-left">
        <div className="max-w-6xl mx-auto space-y-20">
          <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col lg:flex-row items-start lg:items-center gap-6">
            <div className="flex flex-wrap items-center gap-3 border border-red-600/50 p-3 px-5 rounded-full bg-red-600/5 backdrop-blur-md">
              <div className="flex items-center gap-2">
                ...
                <span className="text-red-500 font-mono text-[10px] tracking-[0.4em] font-black uppercase text-left">CLEARANCE</span>
              </div>
              <div className="flex items-center gap-3 border-l border-red-600/50 pl-4 font-mono text-[10px] text-white uppercase font-black text-left">
                <Activity size={12} className="text-red-500 animate-pulse" /> ADMIN NODE SEC-04
              </div>
            </div>
          ...
          <section className="space-y-6 border-l-2 border-red-600 pl-6">
            ...
            <p className="max-w-xl text-slate-500 font-bold leading-relaxed italic uppercase text-xs tracking-tight">Intelligence on operational divergence and system decay. Access restricted to leadership.</p>
          </section>

          <div className="grid gap-6">
            {BRIEFINGS.map((brief) => (
              <motion.div key={brief.id} initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} className="group relative bg-slate-900/10 border border-slate-900 p-8 md:p-10 hover:border-red-600/50 transition-all flex flex-col md:flex-row justify-between items-start md:items-center gap-8 overflow-hidden">
                ...
                <div className="space-y-6 z-10 text-left">
                  ...
                  <div className="space-y-3 text-left">
                    <h2 className="text-3xl md:text-5xl font-black italic uppercase tracking-tighter group-hover:text-red-600 transition-colors leading-none text-white">{brief.title}</h2>
                    <div className="flex flex-wrap items-center gap-3 font-mono text-[9px] font-black tracking-[0.15em] italic text-slate-400">
                       <span>CLASS {brief.classification}</span><span>•</span><span>{brief.pages} PAGES</span>
                    </div>
                    <p className="text-slate-500 text-sm font-medium italic max-w-xl leading-relaxed uppercase tracking-tight">{brief.desc}</p>
                    <div className="text-[8px] font-mono text-red-600/60 uppercase tracking-widest font-black italic">IDENTIFIED {brief.vulnerability}</div>
                  </div>
                </div>
          ...
          <section className="bg-red-600 p-12 md:p-20 text-center space-y-8 relative overflow-hidden shadow-2xl">
             <div className="absolute top-0 right-0 p-4 opacity-5"><Zap size={240} /></div>
             <div className="relative z-10 space-y-6">
                <h2 className="text-5xl md:text-7xl font-black uppercase italic tracking-tighter leading-none text-white text-center">INCOMPLETE DATA <br /> IS SYSTEM RISK</h2>
                <div className="pt-6 text-center"><button onClick={() => { window.location.href = '/pulse-check/assessment'; }} className="bg-black text-white px-14 py-7 font-black uppercase text-[12px] tracking-[0.5em] hover:bg-white hover:text-black transition-all">INITIALIZE DIAGNOSTIC</button></div>
             </div>
          </section>
        </div>
      </main>
