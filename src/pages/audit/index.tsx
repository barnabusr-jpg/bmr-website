  ...
  const generateRecommendation = (): Recommendation => {
    ...
    if (shadowVal !== '0' && shadowVal !== 'undefined') {
      return {
        ...
        confidence: 0.95,
        icon: ShieldCheck,
        rationale: ["Critical Shadow AI infiltration detected.", "Unsanctioned tools bypassing governance.", "Immediate containment required."],
    ...
    if (reworkVal > 1.5) {
      return {
        ...
        confidence: 0.92,
        icon: Zap,
        rationale: ["Verify:Serve ratio indicates profit hemorrhage.", "Human correction costs exceed AI gains.", "Structural logic hardening required."],
    ...
    return {
      ...
      confidence: 0.88,
      icon: Users,
      rationale: [`Team manual capability is at ${expertiseVal}%.`, "Knowledge collapse risk detected.", "Hardening required to restore proficiency."],
  ...
  if (state.isComplete) {
    ...
    return (
      <div className="min-h-screen bg-slate-950 text-white font-sans">
        <Head><title>BMR | Protocol Recommendation</title></Head>
        <Header />
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-12">
            ...
            <div className="border border-slate-800 bg-slate-900/50 p-8 md:p-12">
              ...
              <div className="flex flex-col sm:flex-row gap-4">
                <button onClick={() => window.location.href='/protocols'} className="flex-1 bg-red-600 text-white font-black py-6 uppercase text-xs tracking-widest hover:bg-white hover:text-black transition-all flex items-center justify-center gap-2">
                  <ShieldCheck className="h-4 w-4" /> Activate {rec.title} <ChevronRight className="h-4 w-4" />
                </button>
                <button className="flex-1 border border-slate-700 text-white font-black py-6 uppercase text-xs tracking-widest flex items-center justify-center gap-2">
        ...
        <AnimatePresence mode="wait">
          {state.step === 0 ? (
            ...
            <motion.div key={questions[state.step - 1].id} initial={{ x: 20, opacity: 0 }} animate={{ x: 0, opacity: 1 }} className="space-y-8">
              ...
              <h2 className="text-4xl font-black uppercase italic tracking-tighter">{questions[state.step - 1].text}</h2>
              <div className="pt-8">
