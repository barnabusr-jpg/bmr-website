          ...
          const synchronizeEngineDataMatrix = async () => {
            ...
            if (activeAudit) {
              ...
              const liveCompletions = {
                EXECUTIVE: isOpDone('EXECUTIVE'),
                TECH_MGMT: isOpDone('TECH_MGMT'),
                OPS_MGMT: isOpDone('OPS_MGMT'),
                SYSTEM_USER: isOpDone('SYSTEM_USER')
              };
              ...
              const freshDBEmails = {
                ...
                SYSTEM_USER: filterIncomingEmail(dbSys)
              };

              setEmails(freshDBEmails);

              const liveTriangulationState: TriangulationState = {
                companyName: targetCompanyName || activeAudit.org_name,
                pillar: targetCalculatedPillar,
                emails: freshDBEmails,
                completions: liveCompletions,
                responses: {
                  EXECUTIVE: completedOps.find(n => n.persona_type?.toUpperCase() === 'EXECUTIVE')?.raw_responses || {},
                  TECH_MGMT: completedOps.find(n => n.persona_type?.toUpperCase() === 'TECHNICAL' || n.persona_type?.toUpperCase() === 'TECH_MGMT')?.raw_responses || {},
                  OPS_MGMT: completedOps.find(n => n.persona_type?.toUpperCase() === 'MANAGERIAL' || n.persona_type?.toUpperCase() === 'OPS_MGMT')?.raw_responses || {},
                  SYSTEM_USER: completedOps.find(n => n.persona_type?.toUpperCase() === 'SYSTEM_USER' || n.persona_type?.toUpperCase() === 'SYS')?.raw_responses || {}
                }
              };

              setTriangulation(liveTriangulationState);
              if (targetCompanyName) {
                window.localStorage.setItem(`bmr_matrix_run_${targetCompanyName}`, JSON.stringify(liveTriangulationState));
              }
            ...
            if (targetCompanyName) {
              const fallbackTriangulationState: TriangulationState = {
              ...
              setTriangulation(fallbackTriangulationState);
              window.localStorage.setItem(`bmr_matrix_run_${targetCompanyName}`, JSON.stringify(fallbackTriangulationState));
              setViewState('HUB');
            }
          };
