...
export default function ForensicVerdict() {
  ...
  // 🛡️ RE-INSERTED TICKER LOGIC (Counts from 0 to current penalty)
  useEffect(() => {
  ...
  return (
    <div className="min-h-screen bg-[#020617] text-slate-200 py-16 px-6 font-sans italic text-left selection:bg-red-600/30 print:bg-white print:text-black">
      ...
      {/* 🛡️ RE-INSERTED FLOATING TICKER (FIXED) */}
      <div className="no-print fixed top-6 right-6 z-[100] bg-red-600 text-white px-6 py-4 border-l-4 border-white shadow-xl italic">
      ...
      <div className="container mx-auto max-w-4xl mt-24 relative italic">
        ...
        <header className="flex justify-between items-end mb-12 border-b-2 border-slate-900 pb-10 italic">
          <div>
            ...
            <div className="space-y-1 mt-6 font-mono text-[11px] uppercase tracking-[0.4em] font-black italic">
              <p className="text-slate-500 italic font-black uppercase">CASE_FILE: BMR-2026-{reportData.id.slice(0,4).toUpperCase()} // STATUS: {activeMetrics?.riskStatus}</p>
              <p className="text-red-600 italic font-black uppercase tracking-widest">AUTHORIZED BY: BMR SOLUTIONS</p>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-16 italic font-black">
          <div className="bg-slate-950 border border-slate-900 p-12 flex flex-col justify-center print:bg-slate-100 italic">
            <div className="text-6xl md:text-7xl font-black italic text-white leading-none tracking-tighter print:text-black italic font-black font-black">
              ${activeMetrics?.totalTax.toLocaleString(undefined, {maximumFractionDigits:0})}
            </div>
            <div className="text-[11px] font-mono text-slate-500 uppercase tracking-widest mt-6 font-black italic uppercase">Annual Labor Waste</div>
          </div>
          <div className="bg-red-950/20 border-2 border-red-600/50 p-12 flex flex-col justify-center border-l-8 border-red-600 print:bg-red-50 italic">
            <div className="text-6xl md:text-7xl font-black text-red-500 leading-none italic tracking-tighter italic font-black">
              ${activeMetrics?.inactionPenalty.toLocaleString(undefined, {maximumFractionDigits:0})}
            </div>
            <div className="text-[11px] font-mono text-red-400 uppercase font-black tracking-widest mt-6 italic font-black uppercase">Total Capital Exposure</div>
          </div>
        </div>
        ...
        <div className="bg-slate-950/40 border border-slate-800 p-12 space-y-16 mb-16 rounded-xl no-print italic font-black">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-16 italic font-black">
            ...
            <div className="space-y-6 italic font-black">
              <div className="flex justify-between items-end italic font-black"><span className="text-[11px] font-mono text-slate-500 uppercase tracking-widest font-black italic">Headcount</span><span className="text-2xl font-black italic text-white italic font-black font-black">{fteCount} FTEs</span></div>
        ...
        <div className="bg-white p-16 flex flex-col md:flex-row justify-between items-center gap-12 group cursor-pointer hover:bg-slate-100 transition-all border-l-[20px] border-red-600 shadow-2xl no-print italic font-black" onClick={() => window.open('https://calendly.com/hello-bmradvisory/forensic-review')}>
          <div className="text-left italic font-black">
            <h4 className="text-black text-5xl font-black italic uppercase tracking-tighter leading-none italic font-black font-black">Eradicate the Tax</h4>
