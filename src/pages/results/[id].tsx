...
export default function ForensicVerdict() {
  ...
  // COST OF DELAY ENGINE: Creates real-time psychological pressure
  useEffect(() => {
  ...
  const activeMetrics = useMemo(() => {
    ...
    // BMR-T1: INDEMNITY ALIGNMENT (Weaponized)
    if (isYes(reportData.resultsMap.EXE_01?.answer) && isNo(reportData.resultsMap.TEC_01?.answer)) {
      const cost = 180000;
      fractures.push({
        code: "BMR-T1",
        impact: "CRITICAL",
        implicatedNodes: ['PHOENIX', 'ATLAS'],
        title: "Indemnity Alignment Fracture",
        finding: "PHOENIX assumes audit rights; ATLAS Node reports zero immutable SIEM logging.",
        directive: "STRUCTURAL_HARDENING_REQUIRED",
        action: "Deploy BMR Forensic snapshots to close the $180K/yr liability gap.",
        cost: cost
      });
      frictionScore += 35;
    ...
    // BMR-M1: SYSTEMIC REWORK TAX (Weaponized)
    if (isYes(reportData.resultsMap.MGR_01?.answer) && isNo(reportData.resultsMap.TEC_01?.answer)) {
      ...
      fractures.push({
        code: "BMR-M1",
        impact: "HIGH",
        implicatedNodes: ['TITAN', 'ATLAS'],
        title: "Validation Latency Fracture",
        finding: "Perceptual gap detected between TITAN assumptions and ATLAS ground truth.",
        directive: "INITIALIZE_REINFORCEMENT_LOOPS",
        action: "Automate cross-node checkpoints to eliminate manual engineering rework.",
        cost: cost,
        isSystemic: true
      });
      frictionScore += 35;
    ...
    const perSecondBleed = dailyBleed / 86400;

    return {
      sfi: Math.min(frictionScore, 100),
      totalTax: cumulativeReworkTax,
      inactionPenalty: cumulativeReworkTax * 1.2, // 12-Month Inaction Multiplier
      dailyBleed,
      currentSessionBleed: perSecondBleed * secondsElapsed,
      fractures,
      nodes: {
    ...
    <div className="min-h-screen bg-[#020617] text-white py-20 px-6 font-sans text-left tracking-tighter leading-none">
      <div className="container mx-auto max-w-4xl">
        ...
        <div className="bg-yellow-600/5 border-y border-yellow-600/20 py-3 mb-12 flex items-center justify-center gap-4">
          ...
          <span className="text-[8px] font-mono text-yellow-600 uppercase tracking-[0.4em] font-bold">
            Provisional Assessment // 360° Triangulation Pending // Fiduciary Discovery in Progress
          </span>
        </div>

        {/* PRIMARY FIDUCIARY CARDS */}
        ...
        <div className="bg-slate-950 border border-slate-900 p-8 mb-8">
          <div className="flex justify-between items-center mb-6">
            ...
            <div className="text-xl font-black italic text-white">${liveSpend.toFixed(1)}M <span className="text-[10px] text-slate-500 not-italic ml-2">SPEND</span></div>
          </div>
          <input 
        ...
        {/* LEGAL DISCLAIMER */}
        <div className="bg-red-900/10 border border-red-900/30 p-8 mb-12">
          <p className="text-[9px] font-mono text-red-500 uppercase tracking-[0.2em] leading-relaxed italic">
            ⚠️ <strong>LEGAL DISCLAIMER:</strong> This audit identifies material misalignments between leadership assumptions and technical reality. Failure to address these fractures may result in regulatory exposure, financial leakage, or operational failure.
          </p>
        </div>
        ...
        <div className="bg-slate-950 border border-slate-900 p-10 mb-12">
          ...
          <div className="flex justify-between items-center max-w-2xl mx-auto relative px-10">
            ...
            {[
              { label: 'PHOENIX (EXE)', id: 'PHOENIX', ok: activeMetrics?.nodes.exe },
              { label: 'TITAN (MGR)', id: 'TITAN', ok: activeMetrics?.nodes.mgr },
              { label: 'ATLAS (TEC)', id: 'ATLAS', ok: activeMetrics?.nodes.tec }
            ].map((n, i) => {
              ...
              return (
                <div key={i} className="relative z-10 flex flex-col items-center gap-4">
                  <div className={`w-16 h-16 rounded-full border-2 flex items-center justify-center transition-all duration-700 ${
                    ...
                    isRed ? 'border-red-600 bg-red-600/20 text-red-600' : 'border-green-500 bg-green-500/10 text-green-500'
                  }`}>
            ...
            <div key={i} className="group">
               ...
               <div className="bg-black border-x border-b border-red-600/30 p-10 flex flex-col md:flex-row justify-between items-start gap-8 relative">
                  ...
                  <div className="flex-1">
                    <p className="text-[10px] font-mono text-red-500 uppercase tracking-[0.4em] mb-4 font-black italic leading-none flex items-center gap-2">
                       <EyeOff size={14} /> 🔒 Classified: Full 360° Audit Preview
                    </p>
                    <p className="text-slate-300 text-sm mb-4 leading-relaxed font-bold">
                      A full BMR Engagement includes:
                    </p>
                    <ul className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-2 text-[10px] text-slate-500 font-mono uppercase tracking-widest leading-none">
                       ...
                       <li className="flex items-center gap-2"><Zap size={10} className="text-red-600" /> Automated Forensic Snapshots</li>
                    </ul>
                  </div>
