...
export default function ForensicVerdict() {
  ...
  // 🛡️ LIVE TICKER STATE (Starts at 0.00 every session)
  ...
  const activeMetrics = useMemo(() => {
    ...
    return {
      ...
      // 🛡️ BLEED CALCULATION: Annual Penalty / Seconds in a Year
      bleedPerSecond: inactionPenalty / (365 * 24 * 60 * 60)
    };
  }, [reportData, liveSpend, fteCount]);

  // 🛡️ LIVE TICKER ENGINE
  useEffect(() => {
    if (!activeMetrics?.bleedPerSecond) return;

    const ticker = setInterval(() => {
      setLiveBleed(prev => prev + (activeMetrics.bleedPerSecond / 10));
    }, 100);

    return () => clearInterval(ticker);
  }, [activeMetrics?.bleedPerSecond]);

  if (loading || !reportData) {
    return (
      <div className="bg-[#020617] min-h-screen text-red-600 flex flex-col items-center justify-center font-mono italic animate-pulse uppercase tracking-[0.5em] text-2xl">
    ...
    <div className="min-h-screen bg-[#020617] text-slate-200 py-16 px-6 font-sans italic text-left selection:bg-red-600/30 print:bg-white print:text-black">
      ...
      <div className="container mx-auto max-w-4xl mt-24 relative italic font-black uppercase">
        ...
        <div className="bg-white p-12 mb-16 border-l-[16px] border-red-600 shadow-2xl print:shadow-none italic text-black">
          ...
          {/* 🛡️ SOFT REMINDER TICKER (Blends with Header) */}
          <div className="flex justify-between items-start mb-10 italic font-black">
            ...
            {/* Subtle Ticker Design */}
            <div className="no-print text-right border-r-4 border-red-600 pr-4">
              <span className="text-[8px] font-mono tracking-[0.2em] text-slate-400 uppercase block mb-1">Live_Inaction_Cost</span>
              <div className="text-xl font-black text-red-600 tracking-tighter tabular-nums">
                +${liveBleed.toLocaleString(undefined, { 
