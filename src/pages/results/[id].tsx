...
import { Lock, Activity, Check } from "lucide-react";
...
function RealTimeLossTicker({ diagnosticCompletedAt, exposure, anomalies, isArchived }: LossTickerProps) {
  ...
  useEffect(() => {
    const targetTimestamp = diagnosticCompletedAt || new Date().toISOString();
    ...
    const lossPerSecond = validExposure / 31536000;
  ...
  }, [diagnosticCompletedAt, exposure, isArchived]);

  return (
    <div ref={displayRef} className="font-mono font-black mt-2 tracking-tighter tabular-nums text-red-500 leading-none block break-keep text-4xl md:text-5xl">
      $0.00
    </div>
  );
}
...
export default function InitialPrescreenPortal() {
  ...
  const genericAnomalies: AnomalyNode[] = useMemo(() => {
    ...
    return [
      { id: `ANOMALY SEGMENT ALPHA // LOSS BASELINE $${(pool * 0.35).toLocaleString(undefined, { maximumFractionDigits: 0 })}`, description: "Diagnostic scan parameters verified. Detailed root cause analytics and process map variations are fully compiled and locked under initial intake protocols.", severity: "SECURE GATE", directive: "Schedule your forensic data briefing to unlock complete segment vectors." },
      { id: `ANOMALY SEGMENT BETA // LOSS BASELINE $${(pool * 0.28).toLocaleString(undefined, { maximumFractionDigits: 0 })}`, description: "Diagnostic scan parameters verified. Detailed root cause analytics and process map variations are fully compiled and locked under initial intake protocols.", severity: "SECURE GATE", directive: "Schedule your forensic data briefing to unlock complete segment vectors." },
      { id: `ANOMALY SEGMENT GAMMA // LOSS BASELINE $${(pool * 0.22).toLocaleString(undefined, { maximumFractionDigits: 0 })}`, description: "Diagnostic scan parameters verified. Detailed root cause analytics and process map variations are fully compiled and locked under initial intake protocols.", severity: "SECURE GATE", directive: "Schedule your forensic data briefing to unlock complete segment vectors." },
      { id: `ANOMALY SEGMENT DELTA // LOSS BASELINE $${(pool * 0.15).toLocaleString(undefined, { maximumFractionDigits: 0 })}`, description: "Diagnostic scan parameters verified. Detailed root cause analytics and process map variations are fully compiled and locked under initial intake protocols.", severity: "SECURE GATE", directive: "Schedule your forensic data briefing to unlock complete segment vectors." }
    ];
  ...
  const fireBriefingSequence = () => {
    ...
    const baseUrl = "https://calendly.com/hello-bmradvisory/forensic-briefing";
    window.open(clientEmail ? `${baseUrl}?email=${clientEmail}` : baseUrl, "_blank");
  };
  ...
  if (!mounted || loading || !audit) {
    return (
      <div className="min-h-screen bg-[#020617] flex flex-col items-center justify-center text-green-500 italic font-black">
        ...
        <p className="font-mono text-xs uppercase tracking-[0.4em]">LOADING DIAGNOSTIC PRESCREEN...</p>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-[#020617] text-white font-sans overflow-x-hidden text-left uppercase italic font-black">
      <nav className="h-28 bg-black/40 backdrop-blur-md border-b border-slate-900/60 px-6 md:px-12 flex items-center justify-between">
        <div>
          ...
          <span className="text-[8px] font-mono uppercase tracking-[0.3em] italic block mt-0.5 text-green-500">PORTAL MODE // DIAGNOSTIC PHASE 1</span>
        </div>
      </nav>
      ...
      <main className="max-w-7xl mx-auto pt-12 md:pt-16 px-6 md:px-12 pb-32 space-y-12">
        <div className="border-l-2 border-slate-800 pl-4 py-1 space-y-1">
          ...
          <p className="text-slate-300 font-sans text-xs leading-relaxed font-black normal-case max-w-4xl">
            Metrics are generated using proportional standard model assumptions indexed to your captured Logic Decay Coefficient of {dbDecay}%. Specific workforce calibration parameters are held inside terminal status using system defaults of $1.2M annual software allocations across an ecosystem of 6 FTE resources.
          </p>
        </div>
        ...
        <div className="bg-white text-black p-8 md:p-14 border-l-[12px] md:border-l-[16px] border-green-600 grid grid-cols-1 md:grid-cols-12 gap-8 items-center shadow-2xl relative">
          <div className="md:col-span-7 flex flex-col justify-between space-y-8 md:space-y-10">
            <div>
              <h1 className="text-4xl sm:text-5xl md:text-7xl font-black tracking-tighter leading-none text-black break-words">EFFICIENCY VERDICT</h1>
              <p className="text-[10px] md:text-[11px] font-mono text-slate-400 tracking-widest mt-2.5">TARGET IDENTIFIER // {audit?.org_name || "EVALUATION CLIENT SYSTEM"}</p>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 pt-6 md:pt-8 border-t border-slate-100 text-left">
              <div>
                ...
                <p className="text-xs font-black mt-2 leading-tight text-slate-900">DECAY INDEX: <span className="text-green-500 text-base">{dbDecay}%</span></p>
              </div>
              <div>
                <span className="text-[9px] font-mono block tracking-wider uppercase text-green-500">PROCESS WASTE TAX</span>
                <p className="text-xs font-black mt-2 leading-tight text-slate-900">LIABILITY TOTAL: <span className="text-green-500 font-mono text-sm">${metrics.totalLaborTaxPool.toLocaleString(undefined, { maximumFractionDigits: 0 })}.</span></p>
              </div>
              <div>
                <span className="text-[9px] font-mono block tracking-wider uppercase text-green-500">PROJECTED ANNUAL EXPOSURE</span>
                <p className="text-xs font-black mt-2 leading-tight text-slate-900">TOTAL CAPITAL RISK: <span className="text-green-500 font-mono text-sm">${(metrics.exposure + metrics.totalLaborTaxPool).toLocaleString(undefined, { maximumFractionDigits: 0 })}.</span></p>
              </div>
            </div>
          </div>
          ...
          <div className="md:col-span-4 flex flex-col justify-center items-start md:items-end text-left md:text-right pt-4 md:pt-0 min-w-[240px] shrink-0">
            ...
            <RealTimeLossTicker diagnosticCompletedAt={audit?.created_at} exposure={metrics.exposure + metrics.totalLaborTaxPool} anomalies={genericAnomalies} isArchived={false} />
            <span className="text-[9px] font-mono text-slate-400 block tracking-wider uppercase mt-1.5 whitespace-nowrap">// REAL TIME LOSS SINCE VERDICT LOCK</span>
          </div>
        </div>
        ...
        {/* ANOMALIES CHART INDEX (TERMINAL ENDPOINT FOR PRESCREEN) */}
        <div className="pt-8 text-left">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {genericAnomalies.map((frac, index) => (
              <div key={`anomaly-node-${index}`} className="border p-8 bg-slate-950/60 border-green-500/20 bg-green-950/5 flex flex-col justify-between relative min-h-[280px]">
        ...
        {/* CLEAN TERMINAL CALL TO ACTION */}
        <div className="bg-white text-black p-10 md:p-14 flex flex-col items-center justify-center group cursor-pointer border-l-[16px] border-green-600 shadow-2xl text-center mt-12 hover:bg-slate-50 transition-all duration-300" onClick={fireBriefingSequence}>
