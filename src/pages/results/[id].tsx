...
export default function ForensicVerdict() {
  ...
  useEffect(() => {
    ...
    const fetchAuditData = async () => {
      // 🛡️ ALIGNMENT: Fetching specifically from the audits table based on ID
      ...
      if (auditRes.data) {
        ...
        // 🛡️ INITIAL CALIBRATION: Set the slider to the spend saved in the DB
  ...
  // 🛡️ THE ENGINE: Calculating unique metrics from your Supabase columns
  const activeMetrics = useMemo(() => {
    if (!reportData || !reportData.audit) return null;
    
    const audit = reportData.audit;
    
    // 1. Pull Saved Forensic Truth
    const dbRework = parseFloat(audit.rework_tax) || 0;
    const dbSpend = parseFloat(audit.ai_spend) || 1.2;
    const dbDecay = parseInt(audit.decay_pct) || 74;

    // 2. Proportional Simulation Logic
    // liveSpend (slider) divided by dbSpend (original) = the scalar
    const multiplier = liveSpend / dbSpend;
    
    // 3. Final Synthesis
    // We multiply the base tax by the growth ratio. 
    // If DB value is a factor (like 0.9), we scale to millions for display.
    const totalTax = dbRework * multiplier * 1000000; 

    return {
      sfi: dbDecay, 
      totalTax: totalTax,
      inactionPenalty: totalTax * 1.2, // 12-month friction projection
      dailyBleed: totalTax / 365,
      currentSessionBleed: ((totalTax / 365) / 86400) * secondsElapsed,
      
      // Signal triggers calibrated to your DB decay_pct
      triggerAlpha: dbDecay > 50, 
      triggerBeta: dbDecay > 70,
      isSystemic: dbDecay > 60,
      isOptimized: dbDecay < 35
    };
  }, [reportData, liveSpend, secondsElapsed]);

  if (loading) return (
    <div className="min-h-screen bg-[#020617] flex items-center justify-center text-red-600 font-mono italic animate-pulse tracking-[0.4em]">
      ...
      <div className="container mx-auto max-w-4xl">
        <header className="flex justify-between items-end border-b border-slate-900 pb-8 mb-4">
          <div className="text-left">
            ...
            <p className="text-[10px] font-mono text-slate-500 uppercase mt-4 tracking-widest font-bold italic leading-none">
              Ref ID: {id?.toString().slice(0,8).toUpperCase()} // Entity: {reportData?.org}
            </p>
          </div>
        ...
        <div className="mb-12">
          <div className="mb-8 border-l-2 border-red-600 pl-4">
            ...
            <p className="text-[10px] font-mono text-red-600 uppercase tracking-widest mt-3 font-bold italic">
              Structural Decay Factor: {activeMetrics?.sfi}%
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {activeMetrics?.triggerAlpha && (
              <div className="bg-slate-950 border border-slate-900 p-8 space-y-4">
                <div className="flex justify-between items-center text-red-600">
                  <Fingerprint size={20} />
                  <span className="text-[9px] font-mono font-black uppercase tracking-widest font-bold">Signal_Alpha</span>
                </div>
                ...
                <p className="text-slate-500 text-xs leading-relaxed font-medium italic">
                  Oversight disconnect detected. Potential impact on indemnity standing and regulatory compliance.
                </p>
              </div>
            )}
            ...
            {activeMetrics?.triggerBeta && (
              <div className="bg-slate-950 border border-slate-900 p-8 space-y-4">
                <div className="flex justify-between items-center text-red-600">
                  <Activity size={20} />
                  <span className="text-[9px] font-mono font-black uppercase tracking-widest font-bold">Signal_Beta</span>
                </div>
                <h4 className="text-white font-black italic uppercase text-lg leading-tight">Efficiency vs. Rework</h4>
                <p className="text-slate-500 text-xs leading-relaxed font-medium italic">
                  ROI friction point identified. Risk of margin preservation and operational scale collapse.
                </p>
              </div>
            )}
            ...
            {activeMetrics?.isSystemic && (
              <div className="md:col-span-2 bg-red-950/10 border border-red-900/50 p-8 space-y-4">
                <div className="flex items-center gap-3 text-red-500">
                  ...
                  <span className="text-[9px] font-mono font-black uppercase tracking-widest font-bold">Critical_Status</span>
                </div>
                <h4 className="text-white font-black italic uppercase text-xl leading-tight text-red-500">Systemic Logic Shear</h4>
                <p className="text-slate-400 text-xs leading-relaxed font-medium italic">
                  Input frames a state of total structural drift. Compounding technical debt is currently accruing at scale.
                </p>
              </div>
            )}
          </div>
        ...
        >
           <div className="text-left">
             ...
             <p className="text-slate-600 text-[10px] font-bold uppercase tracking-widest mt-2 italic">Schedule your clinical briefing to verify Ref ID: {id?.toString().slice(0,8).toUpperCase()}.</p>
           </div>
