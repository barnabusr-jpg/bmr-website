...
export default function ForensicVerdict() {
  ...
  const [liveSpend, setLiveSpend] = useState<number>(1.2); // State for the slider
  ...
  // RE-CALCULATION ENGINE: Fires whenever liveSpend or report changes
  const crunch = () => {
    ...
    const fractures = [];
    const baseTaxMultiplier = parseFloat(String(report.rawAudit.rework_tax || "0.9").replace(/[^0-9.]/g, '')) || 0.9;
    ...
    // BMR-T1: INDEMNITY
    if (isYes(report.resultsMap.EXE_01?.answer) && isNo(report.resultsMap.TEC_01?.answer)) {
      const cost = 180000;
      fractures.push({
        code: "BMR-T1",
        impact: "CRITICAL",
        ...
        finding: "Executive policy assumes audit rights; Technical Node reporting gaps detected.",
    ...
    // BMR-M1: SYSTEMIC REWORK TAX
    if (isYes(report.resultsMap.MGR_01?.answer) && isNo(report.resultsMap.TEC_01?.answer)) {
      ...
      fractures.push({
        ...
        finding: "Managerial validation claims exist, but Technical Node reports structural gaps.",
    ...
    return {
      ...
      fractures,
      nodes: {
        exe: report.rawNodes.find((n:any) => n.persona_type?.toUpperCase().includes('EXE'))?.status === 'completed',
        mgr: report.rawNodes.find((n:any) => n.persona_type?.toUpperCase().includes('MGR'))?.status === 'completed',
        tec: report.rawNodes.find((n:any) => n.persona_type?.toUpperCase().includes('TEC'))?.status === 'completed',
      }
    };
  };

  const activeMetrics = crunch();
    ...
    <div className="min-h-screen bg-[#020617] text-white py-20 px-6 font-sans text-left tracking-tighter leading-none">
      <div className="container mx-auto max-w-4xl">
        ...
        <div className="bg-slate-950 border border-slate-900 p-8 mb-8">
          ...
          />
          <div className="flex justify-between text-[8px] font-mono text-slate-600 mt-2 tracking-widest uppercase">
            <span>$0.1M</span>
            <span>Scale Baseline AI Investment</span>
            <span>$10M</span>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
          ...
          {activeMetrics?.fractures.some((f:any) => f.isSystemic) && (
            <div className="bg-red-600/10 border border-red-600 p-8 flex flex-col justify-center relative overflow-hidden">
               ...
               <div className="text-2xl font-black text-red-500 leading-none uppercase italic leading-none">Systemic Gap Liability</div>
               <p className="text-[10px] text-slate-400 mt-2 font-mono uppercase font-bold tracking-tighter">
                 ${(activeMetrics?.fractures.find((f:any) => f.isSystemic)?.cost / 1000).toFixed(0)}K Rework Tax @ ${liveSpend}M
               </p>
            </div>
          )}
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-1 mb-12 text-left">
          <div className="col-span-2 bg-black border border-slate-900 p-12 relative overflow-hidden text-left">
            ...
            <div className="mt-10 h-2 w-full bg-slate-900 overflow-hidden">
              <motion.div animate={{ width: `${activeMetrics?.sfi}%` }} className="h-full bg-red-600" />
            </div>
          </div>
              ...
              return (
                <div key={i} className="relative z-10 flex flex-col items-center gap-4">
                  <div className={`w-16 h-16 rounded-full border-2 flex items-center justify-center transition-all duration-700 ${
                    ...
                    isRed ? 'border-red-600 bg-red-600/20 text-red-600' :
                    isYellow ? 'border-yellow-500 bg-yellow-500/10 text-yellow-500' :
                    'border-green-500 bg-green-500/10 text-green-500'
                  }`}>
                    {!n.ok ? <Activity size={24} /> : isRed ? <AlertTriangle size={24} /> : isYellow ? <Zap size={24} /> : <ShieldCheck size={24} />}
                  </div>
