...
export default function ForensicVerdict() {
  ...
  const activeMetrics = useMemo(() => {
    ...
    // 1. DATA INPUTS FROM THEIR ANSWERS
    const dbDecay = parseInt(reportData.decay_pct) || 0; 
    const currentSpend = liveSpend * 1000000;

    // 2. CAPACITY LOSS (Labor Waste)
    // Their reported decay (0-100) dictates how much of their FTE effort is "rework"
    const reworkPercent = (dbDecay / 100) * 0.4; 
    const annualReworkTax = (fteCount * reworkPercent) * 160000 * 1.3;

    // 3. 🛡️ THE INACOUNTER (Dynamic Forensic Model)
    // We convert their Decay score into a risk multiplier based on their data.
    // A 100% decay score maps to a 35% capital risk baseline.
    const dynamicMultiplier = (dbDecay / 100) * 0.35; 
    
    // The final penalty: (Risk % of Spend) + a 15% complexity/time premium
  ...
  if (loading || !reportData) return (
    <div className="bg-[#020617] min-h-screen text-red-600 flex items-center justify-center font-mono italic animate-pulse uppercase tracking-[0.5em] text-2xl">
      ...
      <div className="container mx-auto max-w-4xl mt-24 relative italic">
        ...
        <header className="flex justify-between items-end mb-12 border-b-2 border-slate-900 pb-10">
          <div>
            ...
            <div className="space-y-1 mt-6 font-mono text-[11px] uppercase tracking-[0.4em] font-black italic">
              <p className="text-slate-500">CASE_FILE: BMR-2026-{reportData.id.slice(0,4).toUpperCase()} // STATUS: {activeMetrics?.riskStatus}</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-16 italic font-black">
          <div className="bg-slate-950 border border-slate-900 p-12 flex flex-col justify-center print:bg-slate-100">
            <div className="text-6xl md:text-7xl font-black italic text-white leading-none tracking-tighter print:text-black italic font-black">
          ...
          <div className="bg-red-950/20 border-2 border-red-600/50 p-12 flex flex-col justify-center border-l-8 border-red-600 print:bg-red-50 italic">
            ...
            <div className="text-[11px] font-mono text-red-400 uppercase font-black tracking-widest mt-6 italic font-black">Total Capital Exposure</div>
          </div>
        </div>

        <div className="bg-slate-950/40 border border-slate-800 p-12 space-y-16 mb-16 rounded-xl no-print italic font-black">
          <div className="flex items-center gap-4 text-red-600 italic"><Sliders size={24} /><h3 className="text-[12px] font-mono font-black uppercase tracking-[0.5em] italic">Forensic Exposure Simulator</h3></div>
