...
export default function ForensicVerdict() {
  ...
  useEffect(() => {
    ...
    const calculateForensics = async () => {
      ...
      if (auditRes.data && nodesRes.data) {
        ...
        // 1. DATA NORMALIZATION (Ensures strings and objects are handled)
        nodes.forEach(n => {
          if (n.raw_responses) {
        ...
        // 2. THE FORENSIC BRAIN (Logic handles "Yes" OR "1")
        let frictionScore = 0;
        const fractures = [];
        const tax = parseFloat(audit.rework_tax || "0");

        const isYes = (val: string) => val === "Yes" || val === "1";
        const isNo = (val: string) => val === "No" || val === "2";
        ...
        if (isYes(results.EXE_01?.answer) && isNo(results.TEC_01?.answer)) {
          fractures.push({
            ...
            directive: "Protocol: BMR-T1 Forensic Logging."
          });
          frictionScore += 35;
        }
        ...
        if (isYes(results.MGR_01?.answer) && isNo(results.TEC_01?.answer)) {
          ...
          fractures.push({
            ...
            directive: "Protocol: BMR-M1 Feedback Loops."
          });
          frictionScore += 35;
        }
  ...
  if (loading) return (
    <div className="min-h-screen bg-[#020617] flex flex-col items-center justify-center space-y-4">
      ...
      <div className="text-red-600 font-mono italic text-xs tracking-[0.3em]">SYNTHESIZING_FORENSIC_DOSSIER...</div>
    </div>
  );
      ...
      <div className="container mx-auto max-w-4xl">
        ...
        <div className="bg-black border border-slate-900 p-12 mb-12 relative overflow-hidden">
          <div className="flex flex-col md:flex-row justify-between items-center gap-12 relative z-10 text-left">
            <div className="flex-1">
              ...
              <div className="mt-8 h-2 w-full bg-slate-900 overflow-hidden"><div className="h-full bg-red-600 transition-all duration-1000" style={{ width: `${report?.sfi}%` }} /></div>
            </div>
        ...
        <div className="space-y-8 mb-20 text-left">
          ...
          {report?.fractures.length > 0 ? report.fractures.map((f: any, i: number) => (
              ...
              <div className="flex flex-col md:flex-row justify-between items-start gap-8">
                <div className="space-y-4 flex-1">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-white font-black uppercase italic text-2xl tracking-tighter leading-none">{f.title}</span>
                  </div>
                  <p className="text-slate-400 italic text-sm leading-relaxed border-l border-slate-800 pl-6">"{f.finding}"</p>
                </div>
                <div className="text-right">
                  <button onClick={() => router.push(`/protocols?id=${id}`)} className="text-red-600 font-mono text-[10px] border-b border-red-600 pb-1 hover:text-white hover:border-white transition-all tracking-widest font-black uppercase leading-none">
                    {f.code} _ ACCESS
                  </button>
                </div>
              </div>
            ...
            <div className="bg-slate-950 border border-slate-900 p-12 text-center border-dashed">
              <AlertCircle className="text-slate-800 mx-auto mb-4" size={32} />
              <p className="text-slate-500 font-mono text-sm uppercase tracking-widest leading-none">No structural fractures detected in current node triangulation.</p>
            </div>
          )}
        </div>
