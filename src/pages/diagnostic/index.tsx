...
const diagnosticQuestions = [
  ...
  { id: 3, text: "Our AI's reliability is a systemic property; it does not depend on a specific power user." },
  ...
  { id: 8, text: "We can update internal AI safety and regulatory rules organization-wide in one deployment cycle." },
  { id: 9, text: "When the system underperforms, we fix the architectural root cause rather than patches." },
  ...
  const submitResults = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        alert("Dispatch failed. Please check connection.");
        ...
        <AnimatePresence mode="wait">
          {step === 0 ? (
            ...
            <motion.div key="question" initial={{ x: 20, opacity: 0 }} animate={{ x: 0, opacity: 1 }}>
              <p className="text-[#14b8a6] font-mono text-sm mb-4 uppercase text-left">Signal {step} of 12</p>
              <h2 className="text-2xl md:text-3xl font-bold mb-10 leading-tight text-left">{diagnosticQuestions[step - 1].text}</h2>
