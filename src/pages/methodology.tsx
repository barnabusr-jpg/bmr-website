...
import { BarChart3, Binary, GitMerge, ShieldAlert, Zap, ChevronRight, Database } from "lucide-react";
      ...
      <main className="pt-44 pb-24 px-6 text-left">
        <div className="max-w-6xl mx-auto space-y-32">
          <section className="space-y-6 border-l-4 border-red-600 pl-8 text-left">
            ...
            <p className="max-w-xl text-slate-500 font-bold leading-relaxed italic uppercase text-xs tracking-tight text-left italic">
              AI systems are not black boxes; they are structural assets prone to fatigue. We calculate the price of neglect.
            </p>
          </section>
            ...
            <div className="grid lg:grid-cols-2 gap-16 items-center">
              <div className="space-y-8 text-left">
                ...
                <div className="p-8 bg-black border border-red-600/20 rounded-sm shadow-2xl text-center">
                  <div className="text-2xl md:text-4xl font-mono font-black italic tracking-tighter text-white">
                    Tr = Σ (Ds × Ch) + Ωl
                  </div>
                </div>
                <p className="text-slate-500 text-[10px] font-bold uppercase tracking-widest text-left font-mono">
                  Ds: Divergence Severity | Ch: Human Correction | Ωl: Systemic Liability
                </p>
              </div>
              <div className="space-y-6 bg-slate-900/40 p-10 border border-slate-800 text-left italic">
                <div className="flex items-center gap-3 text-left">
                    <Binary className="text-red-600" size={18} />
                    <span className="text-[10px] font-mono text-slate-400 uppercase tracking-widest font-black text-left">LOGIC ARCHITECTURE</span>
                </div>
                <p className="text-slate-400 text-sm font-medium leading-relaxed uppercase tracking-tight text-left">
                  BMR tracks Divergence Velocity—how fast your AI is losing contact with reality—allowing recovery before the system goes terminal.
                </p>
              </div>
            </div>
          </section>
            ...
            {PHASES.map((phase, i) => (
              <motion.div key={phase.id} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }} className="group p-8 bg-slate-900/10 border border-slate-900 hover:border-red-600/50 transition-all text-left">
                <div className="space-y-6 text-left">
                  ...
                  <h3 className="text-2xl font-black italic uppercase tracking-tighter group-hover:text-red-600 transition-colors text-left text-white">{phase.title}</h3>
                  <p className="text-slate-500 text-[11px] font-bold uppercase tracking-widest leading-relaxed italic text-left">
                    {phase.desc}
                  </p>
                </div>
              </motion.div>
          ...
          <div className="pt-16 border-t border-slate-900 flex flex-col items-center text-center space-y-4 opacity-30">
            ...
            <p className="text-[10px] font-mono text-slate-500 uppercase tracking-[0.6em] max-w-lg italic font-bold text-center underline">CLASSIFIED BMR DATA NODE-SEC-04</p>
          </div>
        </div>
      </main>
      <Footer />
