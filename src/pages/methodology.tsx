    ...
    <div className="min-h-screen bg-[#020617] text-white font-sans selection:bg-red-950 selection:text-red-200 overflow-x-hidden">
      ...
      <main className="pt-32 sm:pt-44 pb-16 sm:pb-24 px-4 sm:px-6 md:px-12 max-w-7xl mx-auto">
        ...
        <section className="mb-24 sm:mb-32">
          <div className="border-l-4 border-red-600 pl-4 sm:pl-8 mb-12 sm:mb-16 text-left">
            ...
            <h1 className="text-[clamp(2.5rem,6vw,5.5rem)] font-black uppercase tracking-tight leading-none mb-6">
              FORENSIC <span className="text-red-500">PHILOSOPHY.</span>
            </h1>
            
            <div className="space-y-6">
              <p className="text-lg sm:text-2xl text-white max-w-4xl leading-relaxed font-semibold">
              ...
              <p className="text-base sm:text-lg text-slate-400 max-w-4xl leading-relaxed font-normal border-l-2 border-slate-800 pl-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 sm:gap-12 items-start">
            <div className="bg-slate-900/40 border border-slate-800/80 p-6 sm:p-10 shadow-lg rounded-sm">
              <h2 className="text-red-500 font-mono text-xs font-semibold mb-4 tracking-widest uppercase">// ORIGIN AND LEADERSHIP</h2>
              <p className="text-slate-300 text-base leading-relaxed font-normal">
                BMR is built on more than twenty years of technology leadership, including ten years at Microsoft and work protecting <span className="text-white font-semibold">U.S. Government Intelligence</span> in secure networks. After handling over six hundred system recoveries, we developed this framework to close the Promise Gap™ and set clear rules before automation affects production. Our approach has helped enterprise clients reduce unplanned downtime by up to 45 percent and lower annual operational costs by hundreds of thousands of dollars by proactively addressing hidden system risks.
              </p>
            </div>
            
            <div className="space-y-4 sm:space-y-6">
              <div className="flex gap-4 sm:gap-6 items-center border-b border-slate-800/80 pb-4 sm:pb-6">
                <Shield className="text-red-500 shrink-0" size={28} />
                <div>
                  <div className="text-white font-bold text-base uppercase tracking-tight">GOVERNMENT-GRADE SECURITY</div>
                  <p className="text-slate-400 text-xs font-mono uppercase tracking-wider">EXPERIENCE IN SECURE CLOUD & AIR-GAPPED NETWORKS</p>
                </div>
              </div>
              
              <div className="flex gap-4 sm:gap-6 items-center border-b border-slate-800/80 pb-4 sm:pb-6">
                <Briefcase className="text-red-500 shrink-0" size={28} />
                <div>
                  <div className="text-white font-bold text-base uppercase tracking-tight">PROVEN SYSTEM RECOVERY</div>
                  <p className="text-slate-400 text-xs font-mono uppercase tracking-wider">TRACK RECORD OF OVER 600 MAJOR SYSTEM RECOVERIES</p>
                </div>
              </div>
              
              <div className="flex gap-4 sm:gap-6 items-center border-b border-slate-800/80 pb-4 sm:pb-6">
                <Scale className="text-red-500 shrink-0" size={28} />
                <div>
                  <div className="text-white font-bold text-base uppercase tracking-tight">PH.D. AND M.A. LEADERSHIP</div>
                  <p className="text-slate-400 text-xs font-mono uppercase tracking-wider">SPECIALIZED EXPERTISE IN ORGANIZATIONAL DESIGN</p>
                </div>
              </div>
            </div>
          </div>
        ...
        <section className="mb-24 sm:mb-32">
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            {nodes.map((node) => (
              <div key={node.id} className="bg-slate-900/40 border border-slate-800/80 p-6 sm:p-8 shadow-lg rounded-sm relative group hover:border-red-500/80 transition-all flex flex-col justify-between">
                <div>
                  <div className="text-red-500 font-mono text-[10px] font-semibold tracking-wider mb-3 uppercase">{node.id} // {node.lens}</div>
                  <h3 className="text-xl font-bold uppercase tracking-tight text-white mb-1">{node.title}</h3>
                  <p className="text-red-500 text-[10px] font-mono font-semibold tracking-wider mb-6 uppercase">{node.focus}</p>
                  
                  <p className="text-slate-300 text-sm normal-case mb-8 leading-relaxed font-normal min-h-[90px]">
                    {node.description}
                  </p>
                </div>
                
                <div className="space-y-3 border-t border-slate-800/80 pt-6">
                  <div className="text-[10px] font-mono text-slate-500 uppercase tracking-wider mb-2">// KEY DELIVERABLES</div>
                  {node.metrics.map((m, j) => (
                    <div key={j} className="flex items-start gap-2 text-xs text-slate-300 font-sans leading-snug">
        ...
        <section className="mb-20 sm:mb-24 max-w-7xl mx-auto">
          <div className="bg-slate-900/40 border-l-4 border-amber-500 p-6 sm:p-8 font-mono text-xs text-left space-y-3 shadow-lg rounded-sm">
            ...
            <p className="text-slate-300 text-xs sm:text-sm font-sans normal-case font-normal leading-relaxed max-w-5xl">
              <strong className="text-white font-semibold">Enterprise Data Insights:</strong> Enterprise data shows that AI automation moves almost twice as fast as the safety rules needed to control it. Deploying autonomous tools without clear, code-based rules makes the <span className="text-white font-semibold underline decoration-amber-500/50">Promise Gap™</span> even wider. BMR turns complex operational problems into clear rules and deployment gates before your automation launches.
            </p>
        ...
        <section className="mb-20 sm:mb-24 bg-slate-900/40 border border-slate-800/80 p-6 sm:p-12 md:p-16 rounded-sm">
          <div className="max-w-4xl">
            <h2 className="text-3xl sm:text-5xl font-black uppercase tracking-tight mb-6 text-white leading-tight">
              THE FOUR-PHASE <span className="text-red-500">DIAGNOSTIC PROCESS.</span>
            </h2>
            <p className="text-base sm:text-lg font-semibold text-slate-200 mb-4 normal-case leading-relaxed">
            ...
            <p className="text-xs sm:text-sm text-slate-400 mb-10 normal-case font-normal leading-relaxed max-w-3xl">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
            {phases.map((p) => (
              <div key={p.step} className="border-l-2 border-slate-700 pl-6 space-y-2">
                <div className="text-red-500 font-mono font-bold text-2xl uppercase">PHASE_{p.step}</div>
                <div className="text-sm font-bold uppercase tracking-wide text-white">{p.title}</div>
                <p className="text-slate-300 text-xs font-sans normal-case font-normal leading-relaxed">{p.detail}</p>
              </div>
            ))}
          </div>
        </section>
        ...
        <section className="mb-24 sm:mb-32 max-w-7xl mx-auto">
          <div className="bg-slate-900/40 border-l-4 border-red-600 p-6 sm:p-8 flex flex-col md:flex-row items-start md:items-center justify-between gap-6 font-mono text-xs rounded-sm">
            <div className="space-y-2">
              <div className="text-red-500 font-semibold tracking-wider flex items-center gap-2 text-xs uppercase">
                <Lock size={16} className="shrink-0" /> ZERO SECURITY FOOTPRINT COMPLIANCE
              </div>
              <p className="text-slate-300 text-xs normal-case font-sans font-normal max-w-4xl leading-relaxed">
            ...
            <span className="text-slate-500 tracking-wider text-[10px] hidden lg:inline shrink-0 uppercase">ISOLATED_EVALUATION_PROTOCOL</span>
          </div>
        </section>

        {/* --- SECTION IV: THE COST OF INACTION --- */}
        <section className="max-w-4xl mx-auto text-center border border-slate-800/80 p-8 sm:p-16 bg-slate-900/40 rounded-sm">
          <AlertCircle className="text-red-500 mx-auto mb-6" size={40} />
          <h2 className="text-3xl sm:text-4xl font-black uppercase tracking-tight mb-6 text-white leading-none">THE COST OF INACTION</h2>
          <p className="text-base sm:text-xl text-slate-300 leading-relaxed mb-8 normal-case font-normal max-w-2xl mx-auto">
          ...
          <div className="flex flex-col items-center gap-4 w-full">
            <button 
              ...
              className="group relative bg-red-600 text-white px-10 sm:px-16 py-5 text-lg font-bold uppercase tracking-wider hover:bg-white hover:text-red-600 transition-all shadow-xl cursor-pointer w-full sm:w-auto text-center"
            >
              EXECUTE STRATEGY
              <Target className="hidden sm:block absolute -top-3 -right-3 text-white group-hover:text-red-600 transition-all" size={24} />
            </button>
              
            <div className="flex items-center gap-2 text-slate-400 font-mono text-[10px] tracking-wider font-semibold uppercase text-center mt-2">
