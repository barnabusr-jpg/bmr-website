...
export default function SOWBuilderStandalone() {
  ...
  return (
    <div className="bg-[#020617] min-h-screen text-slate-200 font-sans tracking-tighter text-left uppercase font-black p-4 sm:p-6 md:p-12 selection:bg-red-600 selection:text-white italic">
      <main className="max-w-7xl mx-auto space-y-6 sm:space-y-8">
        
        <div className="border-b border-slate-900 pb-4 sm:pb-6 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div>
            <h1 className="text-3xl sm:text-4xl md:text-6xl font-black text-white uppercase tracking-tighter leading-tight italic break-words">
              STATEMENT OF WORK <span className="text-red-600">BUILDER</span>
            </h1>
            <span className="text-[9px] sm:text-[10px] font-mono font-black text-zinc-500 tracking-widest block mt-1.5 not-italic">
          ...
          {diagnosticData && (
            <button
              ...
              className="w-full md:w-auto bg-red-600 text-white font-sans font-black px-5 sm:px-6 py-3.5 sm:py-4 rounded-xs text-xs tracking-widest flex items-center justify-center gap-2 hover:bg-white hover:text-black transition-all disabled:opacity-30 cursor-pointer shadow-lg border-none"
            >
              <Download size={14} /> {isGeneratingPdf ? "GENERATING..." : "EXPORT SOW DOCUMENT (PDF)"}
            </button>
          )}
        </div>

        {error && (
          <div className="border border-red-900 bg-red-950/10 p-4 sm:p-6 flex items-start gap-3 not-italic text-slate-400 font-sans text-xs sm:text-sm tracking-normal font-normal normal-case rounded-sm">
            <ShieldAlert size={20} className="text-red-600 shrink-0 mt-0.5" />
            <div>
              <p className="font-mono uppercase font-black text-[11px] sm:text-xs text-red-500 tracking-wider mb-1">System Awaiting Matrix Payload Verification</p>
              {error}
            </div>
          </div>
        )}
        ...
        {diagnosticData && (
          <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 sm:gap-8 items-start">
            ...
            <div className="lg:col-span-1 border border-slate-900 bg-slate-950/50 p-4 sm:p-6 rounded-sm space-y-4 sm:space-y-6 font-mono not-italic text-xs">
              <div>
                <span className="text-[9px] block text-slate-600 uppercase tracking-widest font-black mb-1">// CONTROL PANEL</span>
                <h4 className="text-white text-xs font-black uppercase tracking-wider mb-2 sm:mb-3">Remediation Toggles</h4>
                  ...
                  {activeRemediations.map((rem: AnomalyRemediationNode) => {
                    ...
                    return (
                      <button
                        ...
                        className={`w-full text-left p-2.5 sm:p-3 border rounded-xs font-mono font-bold tracking-tight text-[10px] sm:text-[11px] transition-all flex items-center justify-between cursor-pointer ${
              ...
              <div className="border-t border-slate-900 pt-3 sm:pt-4">
            ...
            <div className="lg:col-span-3 space-y-6">
              ...
              <div className="border border-slate-900 bg-slate-950/40 p-4 sm:p-6 rounded-sm flex flex-col sm:flex-row sm:items-center justify-between gap-4 font-mono not-italic text-xs text-zinc-400">
                <div>
                  <span className="text-[9px] block text-slate-600 uppercase tracking-widest font-black mb-1">// COGNITIVE TARGET ENTITY CODE</span>
                  <strong className="text-white text-base sm:text-lg font-sans italic font-black tracking-tighter break-words">{diagnosticData.org?.replace(/_/g, ' ')}</strong>
                </div>
                <div className="text-left sm:text-right">
                  ...
                  <span className="text-green-500 font-black tracking-widest uppercase flex items-center sm:justify-end gap-1.5 text-[10px] sm:text-xs">
              ...
              {forensicAnalytics && (
                <div className="border border-slate-900 bg-black/60 p-4 sm:p-6 rounded-sm grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 sm:gap-6 font-mono not-italic text-xs">
                  <div className="sm:col-span-2 md:col-span-4 border-b border-slate-900 pb-2 flex items-center gap-2">
                    <BarChart2 size={14} className="text-red-500 shrink-0" />
                    <span className="text-white font-black tracking-wider text-[9px] sm:text-[10px]">// FORENSIC EVIDENCE SPECIFICATIONS</span>
                  </div>

                  <div>
                    <span className="text-slate-600 block text-[8px] sm:text-[9px] font-black uppercase tracking-widest">RELIABILITY INDEX</span>
                    ...
                    <p className="text-[9px] sm:text-[10px] text-zinc-500 tracking-tight mt-0.5 sm:mt-1 uppercase">FROM {forensicAnalytics.sampleSize} VECTOR POINTS.</p>
                  </div>

                  <div>
                    <span className="text-slate-600 block text-[8px] sm:text-[9px] font-black uppercase tracking-widest">DOMINANT BASIS</span>
                    <div className="flex items-center gap-1.5 mt-1.5 sm:mt-2 text-white font-black tracking-tight text-[10px] sm:text-[11px] break-words">
                  ...
                  <div>
                    <span className="text-slate-600 block text-[8px] sm:text-[9px] font-black uppercase tracking-widest">PRIMARY VECTOR DRIVER</span>
                    <div className="flex items-center gap-1.5 mt-1.5 sm:mt-2 text-white font-black tracking-tight text-[10px] sm:text-[11px] break-words">
                  ...
                  <div>
                    <span className="text-slate-600 block text-[8px] sm:text-[9px] font-black uppercase tracking-widest">COGNITION POSTURE STATE</span>
                    <div className="flex items-center gap-1.5 mt-1.5 sm:mt-2 text-white font-black tracking-tight text-[10px] sm:text-[11px] break-words">
              ...
              {filteredRemediations.length === 0 && (
                <div className="p-8 sm:p-12 border border-dashed border-slate-900 text-center text-slate-600 text-xs sm:text-sm font-sans not-italic font-medium">
              ...
              <div className="space-y-6">
                {filteredRemediations.map((anomaly: AnomalyRemediationNode, idx: number) => (
                  <div key={idx} className="border border-slate-900 bg-slate-950/20 p-5 sm:p-8 rounded-sm space-y-4 sm:space-y-6">
                    <div className="border-b border-slate-900 pb-3 sm:pb-4 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2">
                      <div>
                        <span className="text-[9px] sm:text-[10px] font-mono text-red-500 block font-black tracking-widest not-italic">// REMEDIATION TRACK CODE: 0{idx + 1}</span>
                        <h2 className="text-xl sm:text-2xl font-black text-white tracking-tighter font-sans break-words">{anomaly.title}</h2>
                        <p className="text-xs text-slate-500 font-sans not-italic normal-case font-medium mt-1">{anomaly.scope}</p>
                      </div>
                      <div className="bg-slate-900 border border-slate-800 px-3 sm:px-4 py-1.5 sm:py-2 font-mono not-italic text-[11px] sm:text-xs text-zinc-300 font-black rounded-xs tracking-wider uppercase self-start sm:self-auto">
                        {anomaly.investment_tier}
                      </div>
                    </div>

                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 sm:gap-8 not-italic normal-case font-sans tracking-normal font-normal text-xs sm:text-sm text-slate-300">
                      <div className="border border-slate-900 bg-black/40 p-4 sm:p-6 rounded-sm space-y-3 sm:space-y-4">
                        <div className="flex items-center gap-2 border-b border-slate-900 pb-2">
                          <Terminal size={16} className="text-red-500 shrink-0" />
                          <h3 className="text-red-500 font-mono text-[11px] sm:text-xs font-black uppercase tracking-widest">// ARCHITECTURAL TECHNICAL RUNBOOK</h3>
                        </div>
                        <p className="text-xs text-slate-400 italic leading-relaxed">
                          <strong className="text-slate-300 block font-bold uppercase tracking-wide text-[9px] sm:text-[10px] font-mono not-italic text-red-500/80 mb-1">Root Cause Profile:</strong> 
                      ...
                      <div className="border border-slate-900 bg-black/40 p-4 sm:p-6 rounded-sm space-y-3 sm:space-y-4">
                        <div className="flex items-center gap-2 border-b border-slate-900 pb-2">
                          ...
                          <h3 className="text-indigo-400 font-mono text-[11px] sm:text-xs font-black uppercase tracking-widest">// PROCEDURAL OPERATIONAL PLAYBOOK</h3>
                        </div>
                        <p className="text-xs text-slate-400 italic leading-relaxed">
                          <strong className="text-slate-300 block font-bold uppercase tracking-wide text-[9px] sm:text-[10px] font-mono not-italic text-indigo-400/80 mb-1">Structural Governance Gap:</strong> 
