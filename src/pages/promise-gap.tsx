...
export default function PromiseGap() {
  ...
  const [step, setStep] = useState(0); // Mandatory: Start at 0 for the Profile Gate
  ...
  const submitResults = async () => {
    ...
    // Vault the baseline for the results page
    ...
    // Redirect to the RESULTS page we built
            ...
            <AnimatePresence mode="wait">
                  ...
                  <Card className="p-10 bg-slate-900/30 border-slate-800 border-2 relative overflow-hidden">
                    ...
                    <form onSubmit={(e) => { e.preventDefault(); setStep(1); }} className="space-y-6">
                      <div className="grid grid-cols-2 gap-4">
                        <input required placeholder="Full Name" className="w-full p-4 bg-slate-950 border border-slate-800 text-sm font-mono" value={formData.name} onChange={(e) => setFormData({...formData, name: e.target.value})} />
                        <input required type="email" placeholder="Email" className="w-full p-4 bg-slate-950 border border-slate-800 text-sm font-mono" value={formData.email} onChange={(e) => setFormData({...formData, email: e.target.value})} />
                      </div>
                      <input required placeholder="Organization" className="w-full p-4 bg-slate-950 border border-slate-800 text-sm font-mono" value={formData.organization} onChange={(e) => setFormData({...formData, organization: e.target.value})} />
                      <div className="grid grid-cols-3 gap-4 pt-4 border-t border-slate-800">
                        <div className="space-y-1">
                          <label className="text-[9px] uppercase font-bold text-slate-500">Authority</label>
                      ...
                      <Button type="submit" className="w-full bg-[#14b8a6] hover:bg-white text-black font-black uppercase tracking-widest h-16">Initialize Observation</Button>
                    </form>
                  </Card>
                </motion.div>
              )}
              ...
              {step > 0 && step <= 12 && (
                <motion.div key="question" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
                  <Card className="p-12 bg-slate-900/30 border-slate-800 border-2 text-center relative">
                    <div className="absolute top-0 left-0 w-1.5 h-full bg-[#14b8a6]"></div>
              ...
              {step === 13 && (
                <motion.div key="results" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                  <Card className="p-12 bg-slate-900/30 border-slate-800 border-2 text-center">
                    ...
                    <Button className="bg-[#14b8a6] hover:bg-white text-black font-black w-full h-16 uppercase tracking-widest" onClick={submitResults} disabled={isSubmitting}>
