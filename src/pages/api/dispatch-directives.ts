  ...
  try {
    ...
    for (const [rawRole, email] of roles) {
      ...
      emailPromises.push(sgMail.send({
        to: targetEmail,
        from: process.env.SENDGRID_FROM_EMAIL || 'hello@bmrsolutions.co',
        ...
        html: `
          <div style="font-family: monospace; background: #020617; color: white; padding: 40px; border: 2px solid #dc2626;">
            <h2 style="color: #dc2626; text-transform: uppercase; margin-bottom: 5px; letter-spacing: 1px;">BMR SOLUTIONS // FORENSIC TRIANGULATION</h2>
            ...
            <p style="font-size: 10px; color: #475569; margin-top: 40px; border-top: 1px solid #1e293b; padding-top: 20px;">CONFIDENTIAL // BMR SOLUTIONS ENCRYPTION ACTIVE</p>
          </div>
        `
      }));
    }
