...
const DIAGNOSTIC_MAPPING: Record<string, { label: string; snippet: string }> = {
  "Value Drain": { 
    ...
    snippet: "Initial patterns suggest that human work may be slowing down the AI. This may indicate that manual oversight is impacting the intended margin profile of the system." 
  },
  "Utility Only": { 
  ...
  const emailHtml = `
    <div style="font-family: sans-serif; color: #020617; max-width: 700px; margin: 0 auto; border: 1px solid #e2e8f0; padding: 40px; border-radius: 12px;">
      ...
      <div style="background-color: #f8fafc; padding: 24px; border-radius: 8px; margin: 24px 0; border: 1px solid #f1f5f9;">
        <p style="margin: 0; font-size: 18px; color: #0f172a;"><strong>${leadName}</strong> | ${leadOrg}</p>
        <p style="margin: 4px 0 0 0; font-size: 14px; color: #64748b;">Contact: ${leadEmail}</p>
      </div>
      <table style="width: 100%; border-collapse: collapse; margin-top: 12px;">
        <thead>
          <tr style="text-align: left; background-color: #f8fafc;">
            ...
            <th style="padding: 12px; border-bottom: 2px solid #14b8a6;">EXPERIENCE</th>
            <th style="padding: 12px; border-bottom: 2px solid #14b8a6;">OBSERVATION*</th>
          </tr>
        </thead>
        <tbody>${resultsTableRows}</tbody>
      </table>
      <div style="margin-top: 40px; text-align: center; border-top: 1px solid #f1f5f9; padding-top: 20px;">
        <p style="font-size: 10px; color: #94a3b8;">*Advisory observations based on directional patterns. © ${new Date().getFullYear()} BMR Advisory</p>
      </div>
    </div>`;
