...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const msg = {
    ...
    html: `` // Keep empty for Outlook stability
  };
  ...
  try {
    ...
    // 2. Webhook Dispatch (Commercial Gate Active)
    ...
    if (WEBHOOK_URL !== 'YOUR_WEBHOOK_URL_HERE') {
      ...
      } catch (err) { console.error("Webhook non-blocking error:", err); }
    }

    // MANDATORY: This specific line stops the hanging spinner on your screen
    return res.status(200).json({ success: true });

  } catch (error: any) {
    console.error('API Error:', error.message);
    // MANDATORY: Tells the browser to show an error instead of spinning forever
