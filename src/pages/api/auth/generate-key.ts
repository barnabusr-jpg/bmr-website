...
// 🛡️ RE-ALIGNED HANDSHAKE: Points to your new BMR-specific key
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const msg = {
    to: email,
    // 🛡️ Uses your Vercel 'SENDGRID_FROM_EMAIL' variable or defaults to hello@
  ...
  try {
    ...
    // Return the challenge so the frontend can verify it
    return res.status(200).json({ success: true, challenge: otp });
  } catch (error: any) {
    // Forensic logging for Vercel troubleshooting
