...
// 🏛️ Restored Navigation Mapping
const NAV_LINKS = [
  { name: 'METHODOLOGY', path: '/methodology' },
  { name: 'BRIEFINGS', path: '/briefings' }, // This serves as the Frameworks/Vault hub
];
...
export default function Header() {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 h-24 bg-[#020617]/90 backdrop-blur-xl border-b border-slate-900 z-[1000] flex items-center justify-between px-6 md:px-12">
      ...
      {/* 🧭 NAVIGATION: Methodology and Briefings Restored */}
      <nav className="hidden md:flex items-center gap-10">
        {NAV_LINKS.map((link) => (
          <Link 
            key={link.path} 
            href={link.path} 
            className="text-[10px] font-black tracking-[0.3em] text-slate-400 hover:text-white transition-colors no-underline uppercase italic"
          >
            {link.name}
          </Link>
        ))}
