...
const ForensicTriageGrid = ({ onLock }: { onLock: (finalValue: number) => void }) => {
  ...
  const calculateValidatedHemorrhage = () => {
    ...
    // IP PROTECTION: Non-linear logarithmic decay curve to mask coefficients
  ...
  return (
    <div className="bg-slate-950 border-2 border-slate-900 p-8 my-8 relative overflow-hidden">
      <div className="flex items-center justify-between mb-8 border-b border-slate-900 pb-4">
        ...
        <div className="text-[10px] font-mono text-slate-700 uppercase italic">Ref: BMR_V3_CAL</div>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-8">
        <div className="space-y-3">
          <label className="block text-[9px] font-black text-slate-500 uppercase tracking-widest">Active Node Density (FTE)</label>
          <input 
            type="number" 
            disabled={isLocked}
            value={params.nodes}
        ...
        <div className="space-y-3">
          <label className="block text-[9px] font-black text-slate-500 uppercase tracking-widest">Systemic Integrity Baseline</label>
          <select 
            disabled={isLocked}
      ...
      <div className="flex justify-between items-center">
        <p className="text-[10px] text-slate-600 italic uppercase font-mono leading-tight max-w-xs">
          *Manual calibration verifies the Rework Tax against confirmed node density.
        </p>
        <button 
          onClick={isLocked ? () => setIsLocked(false) : handleLock}
          className={`${isLocked ? 'text-slate-500' : 'bg-[#14b8a6] text-slate-950'} px-6 py-3 font-black uppercase text-[10px] tracking-widest transition-all flex items-center gap-2`}
        >
