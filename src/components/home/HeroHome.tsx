...
import { useRouter } from 'next/router'; // 🛡️ Ensure this is from 'next/router'
import { ArrowRight } from "lucide-react";

export default function HeroHome() {
  const router = useRouter();

  // ⚡ LAUNCH HANDLER: Adds ?intake=active to the URL without a page refresh
  const initiateDiagnostic = () => {
    router.push(
      {
        pathname: router.pathname,
        query: { ...router.query, intake: 'active' },
      },
      undefined,
      { shallow: true } // Keeps the user on the home page while launching the modal
    );
  };

  return (
    <section className="bg-[#020617] text-white pt-32 pb-16 md:pt-56 md:pb-32 w-full overflow-hidden selection:bg-red-600/30">
      <div className="max-w-[1280px] mx-auto px-6 md:px-10 flex flex-col lg:flex-row items-start justify-between gap-12 lg:gap-16">
        ...
        <div className="w-full lg:w-[55%] text-left shrink-0">
          <div className="border-l-[6px] border-red-600 pl-6 md:pl-10 mb-10 md:mb-16">
            <p className="text-red-600 font-black text-[10px] md:text-xs tracking-[0.5em] mb-5 font-mono italic uppercase">
          ...
          <div className="border-t border-[#1e293b] pt-10 max-w-[550px]">
            <span className="text-red-600 font-black text-[10px] md:text-[11px] tracking-[0.4em] font-mono italic uppercase">Systemic Decay:</span>
            <p className="text-slate-400 text-xl md:text-2xl font-medium italic mt-4 font-sans uppercase leading-tight italic">
              Friction created when strategic intent and machine execution layers decouple.
            </p>
            
            {/* ⚡ UPDATED CTA: Triggers the Modal Launch */}
            <button 
              onClick={initiateDiagnostic} 
              className="mt-10 bg-red-600 text-white py-5 px-10 md:py-6 md:px-12 font-black uppercase tracking-[0.3em] text-[11px] md:text-xs hover:bg-white hover:text-black transition-all flex items-center gap-4 italic"
            >
        ...
        {/* RIGHT COLUMN: THE REWORK TAX (FIXED PUNCTUATION) */}
        <div className="w-full lg:w-[40%] mt-8 lg:mt-24 shrink-0">
          <div className="bg-[#0f172a]/40 border-2 border-[#1e293b] p-8 md:p-14 text-left relative overflow-hidden shadow-2xl">
            ...
            <h2 className="text-3xl md:text-5xl font-black italic uppercase m-0 font-sans leading-none text-white tracking-tighter">
              The <span className="text-red-600 underline decoration-2 underline-offset-8 italic">Rework Tax</span>
            </h2>

            <div className="border-l-2 border-red-600 pl-6 md:pl-8 mt-10">
              <p className="text-slate-400 italic font-sans text-lg md:text-xl leading-relaxed font-bold uppercase italic">
              ...
              <div className="mt-8 border-t border-[#1e293b] pt-8">
                <p className="text-red-600 font-black uppercase font-sans text-xl md:text-2xl italic tracking-tighter">
                  YOU HAVE <span className="text-white italic">HIDDEN CAPACITY.</span>
                </p>
              </div>
            </div>
          </div>
