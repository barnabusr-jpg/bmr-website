  ...
  const activeQuestions = useMemo(() => { 
    ...
    if (normalizedRole === 'SYSTEM_USER' || normalizedRole.includes('USER') || normalizedRole.includes('SYS')) { 
      ...
      filtered = rawList.filter(q => { 
        ...
        const nodeStr = String(q.target_node ?? '').toUpperCase(); 

        const nodeOk = 
          !q.target_node || 
          allowedTarget.some(tok => nodeStr.includes(tok)); 

        return pillarOk && nodeOk; 
      }); 

      if (filtered.length === 0) { 
        const available = [...new Set(rawList.map(q => String(q.target_node ?? '')).filter(Boolean))]; 
        console.error("[MATRIX_FILTER_EMPTY][SYSTEM_USER]", { 
          normalizedRole, 
          availableTargetNodes: available.slice(0, 50), 
        }); 
      } 
