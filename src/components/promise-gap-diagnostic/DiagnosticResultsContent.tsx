...
const DiagnosticResultsContent = () => {
  ...
  if (!data) return <div className="py-20 text-center animate-pulse text-slate-500 uppercase tracking-widest text-xs">Initialising Forensic Topology...</div>;

  const totalDisplacement = (data.HAI?.aggregate || 0) + (data.AVS?.aggregate || 0) + (data.IGF?.aggregate || 0);
  
  const getSeverity = (score: number) => {
    if (score >= 120) return { label: "OPTIMIZED STEWARDSHIP", color: "#10B981" };
    if (score >= 80) return { label: "PROTOCOLIZED DRIFT", color: "#00F2FF" };
  ...
  const chartData = [
    { zone: 'Pillar 1', value: data.HAI?.aggregate || 0 },
    { zone: 'Pillar 2', value: data.AVS?.aggregate || 0 },
    { zone: 'Pillar 3', value: data.IGF?.aggregate || 0 },
  ];
  ...
  return (
    <div className="py-8 space-y-12">
      ...
      <div className="pt-12 border-t border-slate-800 relative">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 opacity-5 pointer-events-none grayscale blur-sm">
