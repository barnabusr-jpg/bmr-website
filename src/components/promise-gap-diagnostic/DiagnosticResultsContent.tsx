    ...
    <div className="max-w-5xl mx-auto py-12 px-6 text-white font-sans selection:bg-red-600/30">
      <div className="border-b border-slate-800 pb-8 mb-12 flex justify-between items-end">
        <div>
          ...
          <p className="text-[10px] text-slate-500 uppercase tracking-[0.3em] font-mono mt-1 italic">Audit ID: {data.organization.toUpperCase()}_0401</p>
        </div>
        ...
        <div className="space-y-16 animate-in fade-in duration-1000">
          ...
          <section className="grid grid-cols-1 md:grid-cols-2 gap-12 items-start">
            <div className="space-y-6">
              <h3 className="text-xl font-black italic uppercase tracking-tight flex items-center gap-2"><TrendingUp className="text-red-600" size={20} /> The Compound Failure</h3>
              <p className="text-sm text-slate-400 leading-relaxed italic">&quot;Your ${result.financials.estimate.toLocaleString()} hemorrhage is a compounding liability scaling to <span className="text-white font-bold">$20.4M</span> by Q4 2026.&quot;</p>
              <button onClick={() => setShowMath(!showMath)} className="text-[10px] font-black uppercase tracking-widest text-red-600 flex items-center gap-2 hover:text-white transition-colors"><Info size={14} /> {showMath ? "Hide" : "View"} Breakdown</button>
            </div>
            <div className="bg-slate-900/40 p-8 border border-slate-800 space-y-6 relative overflow-hidden text-[10px] font-mono uppercase tracking-widest">
              <div className="flex justify-between items-center border-b border-slate-800 pb-4"><span>Visible Tip</span><span className="text-white">$1.2M</span></div>
              <div className="flex justify-between items-center"><span>Total Liability</span><span className="text-red-600">$20.4M</span></div>
            </div>
          </section>
          ...
          {showMath && (
            <div className="bg-slate-950 p-10 border border-slate-900 grid grid-cols-3 gap-8 text-center uppercase tracking-widest font-mono animate-in slide-in-from-top-4">
              ...
              <div><span className="text-[9px] text-slate-600">Strategic Gap</span><div className="text-xl font-bold text-white">${result.financials.misalignment.toLocaleString()}</div></div>
            </div>
          )}
          ...
          <div className="bg-red-600 p-12 text-center shadow-2xl flex flex-col md:flex-row items-center justify-between gap-8">
            ...
            <div className="flex flex-col gap-4">
               <button className="bg-black text-white px-12 py-5 font-black uppercase text-xs tracking-[0.4em] flex items-center gap-4 hover:bg-slate-800 transition-all">Schedule Deep Dive <ArrowRight size={18} /></button>
               <button onClick={handleDownload} disabled={isDownloading} className="text-[10px] font-black uppercase tracking-widest text-white/50 hover:text-white flex items-center justify-center gap-2">
                 <Download size={14} className={isDownloading ? "animate-bounce" : ""} /> {isDownloading ? "Generating..." : "Download Dossier"}
               </button>
            </div>
          </div>
        </div>
