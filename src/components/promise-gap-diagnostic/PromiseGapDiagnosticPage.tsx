...
// --- CORE DIAGNOSTIC DATA: SHIELDED LABELS (IP PROTECTED) ---
  ...
  useEffect(() => {
    if (step === 13 && !isSubmitting) {
      const submitResults = async () => {
        ...
        try {
          // Full data stored locally for the Results page
              ...
              <Card className="p-10 bg-slate-900/30 border-slate-800 backdrop-blur-sm shadow-2xl">
                ...
                <div className="border-l-2 border-[#00F2FF] bg-[#0A1F33]/40 p-6 mb-8">
                  ...
                  <p className="text-slate-300 text-xs leading-relaxed italic">
                    If a protocol is unknown or not applicable to your role, please select <span className="text-white font-bold text-[#00F2FF]">Stage 1 (Reactive)</span>. This provides the baseline for forensic stabilization.
                  </p>
                </div>
                ...
                >
                  ...
                  <button type="submit" className="w-full bg-[#00F2FF] text-[#020617] font-black h-16 uppercase tracking-widest hover:bg-[#00d8e4] transition-all">Begin Observation</button>
                </form>
              </Card>
            </motion.div>
          )}
          ...
          {step === 13 && (
            <motion.div key="dispatch" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
              <Card className="p-12 bg-slate-900/30 border-slate-800 text-center backdrop-blur-sm shadow-2xl">
                ...
                <p className="text-[#00F2FF] text-[10px] uppercase tracking-[0.3em] font-bold mb-10 animate-pulse italic">Dispatching Maturity Benchmark to {formData.email}</p>
