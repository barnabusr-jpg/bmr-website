...
// SYSTEMIC STATES - THE SOURCE OF TRUTH FOR BUTTON LABELS
  ...
  return (
    <div className="flex flex-col items-center gap-3">
      <div className={`h-14 w-14 rounded-full flex items-center justify-center border-2 relative transition-all duration-500 ${
        ...
        {isCompleted ? <Check className="h-6 w-6 text-white" /> : <span className={`text-xs font-bold ${isActive ? "text-white" : "text-slate-600"}`}>{label[0]}</span>}
      </div>
      <div className={`text-xs font-bold uppercase tracking-[0.2em] ${isActive || isCompleted ? "text-[#14b8a6]" : "text-slate-600"}`}>
        {label}
      </div>
    </div>
  );
  ...
  return (
    <div className="w-full max-w-4xl mx-auto py-12">
      ...
      <AnimatePresence mode="wait">
        {step === 0 && (
          <motion.div key="intake" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
              ...
              <div className="grid grid-cols-1 gap-4 max-w-md mx-auto">
                {systemicStates.map((state) => (
                  <Button 
                    key={state} 
                    variant="outline" 
                    className="py-8 text-lg font-light border-slate-800 hover:border-[#14b8a6] hover:bg-[#14b8a6]/10 transition-all" 
