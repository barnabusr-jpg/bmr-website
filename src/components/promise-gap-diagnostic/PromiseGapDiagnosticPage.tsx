...
function LensIndicator({ label, acronym, isActive, isCompleted }: { label: string; acronym: string; isActive: boolean; isCompleted: boolean }) {
  return (
    <div className="flex flex-col items-center gap-3">
      <div className={`h-14 w-14 rounded-full flex items-center justify-center border-2 transition-all duration-500 ${isActive || isCompleted ? "bg-[#14b8a6] border-[#14b8a6] shadow-[0_0_15px_rgba(20,184,166,0.2)]" : "bg-slate-900 border-slate-800"}`}>
...
export default function PromiseGapDiagnosticPage() {
  ...
  // Automated trigger when user reaches the synthesis screen
  useEffect(() => {
              ...
              <Card className="p-12 bg-slate-900/30 border-slate-800 border-2 text-center relative overflow-hidden">
                ...
                <div className="grid grid-cols-1 gap-4 max-w-md mx-auto">
                  {frequencyScale.map((f) => (
                    <Button key={f.value} variant="outline" className="py-8 text-lg font-light border-slate-800 hover:border-[#14b8a6] hover:bg-[#14b8a6]/5 transition-all text-slate-300 hover:text-white uppercase tracking-widest text-xs" onClick={() => handleAnswer(f.value)}>{f.label}</Button>
                  ))}
                </div>
              </Card>
            </motion.div>
