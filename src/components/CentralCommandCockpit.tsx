...
interface CockpitProps {
  ...
  onSuccess?: () => void; // Optional callback to trigger a global data refresh
}
...
export default function CentralCommandCockpit({
  ...
  // 1. Form and Workspace State Controls
  ...
  // 2. Loading & Notification States (The Guardrails)
  ...
  // 🚀 BUTTON 1 TRIGGER: Deploy Stakeholder Diagnostics Directive
  const handleDeployDiagnostics = async (e: React.FormEvent) => {
    e.preventDefault();
    if (isProcessing) return; // Hardened block against double-submits
    ...
    try {
      const response = await fetch('/api/dispatch-directives', {
        ...
        body: JSON.stringify({
          ...
          orgName,
          parentAuditId,
          emails // Uses the exact structural format verified by our test runs
        })
      });
      ...
      if (onSuccess) onSuccess(); // Instant screen state reconciliation
  ...
  // 🔓 BUTTON 2 TRIGGER: Dispatch Secure Vault Document Links
  const handleDispatchVaultLinks = async () => {
    ...
    try {
      const response = await fetch('/api/send-vault-link', {
        ...
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          auditId: parentAuditId, // Standardized object key matching the backend handler
          orgName,
          email: emails.manager,  // Targets the master managerial representative
  ...
  return (
    <div style={{ background: '#020617', border: '2px solid #dc2626', padding: '30px', fontFamily: 'monospace', color: '#ffffff', maxWidth: '700px', margin: '20px auto', boxSizing: 'border-box' }}>
