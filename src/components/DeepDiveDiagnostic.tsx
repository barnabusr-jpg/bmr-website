...
export default function DeepDiveDiagnostic({ operatorId, userLens }: { operatorId: string, userLens: string }) {
  ...
  const handleNext = async (answer: string) => {
    ...
    if (currentIdx < lensQuestions.length - 1) {
      ...
      // 🚨 CREATE AUDIT RECORD: This makes the test visible in the Admin Dashboard
      const reworkTaxCalc = (Math.random() * (1.8 - 0.4) + 0.4).toFixed(1);

      await supabase.from('audits').insert([{
        operator_id: operatorId,
        rework_tax: reworkTaxCalc,
        status: 'PENDING_RELEASE',
        lens: LENS_SIGNAL
      }]);

      localStorage.setItem("bmr_active_lens", LENS_SIGNAL);
      localStorage.setItem("bmr_final_tax", reworkTaxCalc);
  ...
  if (!isAuthorized) return <div className="min-h-screen bg-[#020617] flex items-center justify-center text-white font-black uppercase italic tracking-tighter font-sans">Node_Locked</div>;
  ...
  return (
    <div className="min-h-screen bg-[#020617] text-white flex flex-col md:flex-row font-sans">
      <aside className="w-full md:w-96 bg-slate-950 border-r border-slate-900 p-10 space-y-12">
        <div className="flex items-center gap-3 text-red-600 font-black uppercase italic text-sm tracking-tighter italic"><BookOpen size={20} /> BMR_FieldGuide_V3</div>
        <div className="p-6 border border-slate-800 bg-slate-900/40 rounded-sm">
           <label className="text-[10px] font-mono text-slate-500 uppercase tracking-widest font-black block mb-2 font-sans">Active_Chapter</label>
           <h4 className="text-white font-bold text-[11px] uppercase font-sans">{lensQuestions[currentIdx]?.chapter}</h4>
        </div>
      </aside>
      <main className="flex-1 p-12 md:p-24 overflow-y-auto bg-[#020617]">
        <div className="max-w-3xl">
          <div className="flex items-center gap-2 text-red-600 font-mono text-[10px] uppercase tracking-widest mb-12 italic font-black font-sans">
             <Activity size={14} className="animate-pulse font-sans" /> Zone_{lensQuestions[currentIdx]?.zone} // Lens_{LENS_SIGNAL}
          </div>
          ...
          <div className="grid grid-cols-1 gap-4 font-sans">
            {["High Confidence", "Moderate Confidence", "Theoretical"].map((opt) => (
              <button key={opt} onClick={() => handleNext(opt)} className="w-full p-8 bg-slate-900/30 border border-slate-800 text-left hover:border-red-600 transition-all flex justify-between items-center group font-black uppercase italic text-slate-400 hover:text-white font-sans">
