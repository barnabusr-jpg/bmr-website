...
/**
 * Statement of Work (SOW) Matrix Table (Page 3)
 * Provides interactive phase/option selection for client web views and hard page breaks for PDF export.
 */
export default function RemediationSowMatrixTable() {
  const { state, updateSowOption } = useDiagnostic();

  // Fail-Closed Gate: Halt execution if state is unverified or incomplete
  if (!state || !state.isComplete) {
    return (
      <div
        className="w-full max-w-5xl mx-auto bg-slate-950 border-2 border-red-600 rounded-lg p-8 text-white font-mono my-8"
        style={{ breakAfter: "page", pageBreakAfter: "always" }}
      >
        <div className="flex items-center gap-3 text-red-500 mb-2">
          <span className="text-xl">⚠️</span>
          <h2 className="text-sm font-bold tracking-widest uppercase">// SECTION REJECTED</h2>
        </div>
        <p className="text-xs text-slate-300">
          INVARIANT FAILURE: Statement of Work matrix cannot render from unverified state.
        </p>
      </div>
    );
  }
  ...
  return (
    <div
      className="space-y-6 text-left print:p-0 print:m-0"
      style={{ breakInside: "avoid", breakAfter: "page", pageBreakAfter: "always" }}
    >
      {/* SECTION HEADER */}
      ...
      {/* SOW MATRIX TABLE */}
      <div className="overflow-x-auto border border-slate-200 bg-white rounded-xs shadow-xs">
          ...
          <tbody className="divide-y divide-slate-200 text-slate-700">
            ...
            <tr className="hover:bg-slate-50/50">
              ...
              <td className="p-3.5 align-top space-y-2">
                ...
                <div className="flex gap-2 print:hidden font-mono text-[10px]">
      ...
      <div className="bg-slate-950 text-white p-5 rounded-xs space-y-3">
        <span className="font-mono text-xs font-bold text-red-700 uppercase tracking-widest block">
          // SECURITY &amp; DATA HANDLING FAST FACTS
        </span>
