...
export default function ConsolidatedDiagnostic() {
  ...
  const getLiveMetrics = () => {
    ...
    return { decay: Math.min(Math.round((1 - (1 / (1 + scaledTotal / (aiSpend * 0.8)))) * 100), 98), rework: (scaledTotal * 0.38).toFixed(1) };
  };
  ...
  const logToDatabase = async (finalMetrics: any) => {
    try {
      ...
      const { data: op } = await supabase.from('operators').upsert({ email: email.toLowerCase(), full_name: operatorName.toUpperCase(), entity_id: ent?.id, persona_type: selectedLens }).select().single();
      const { data: auditData } = await supabase.from('audits').upsert([{ org_name: entityName.toUpperCase(), lead_email: email.toLowerCase(), operator_id: op?.id, sector, ai_spend: aiSpend, decay_pct: finalMetrics.decay, rework_tax: parseFloat(finalMetrics.rework), raw_responses: answers, status: 'LEAD' }]).select().single();
      return auditData?.id;
    } catch (e) { return null; }
  };
      ...
      <AnimatePresence mode="wait">
        {step === 'triage' && (
          <motion.div key="triage" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-16 text-center">
            <h1 className="text-[50px] sm:text-[70px] md:text-[100px] font-black uppercase italic tracking-tighter text-white leading-[0.85]">
              THE LOGIC <br /> <span className="text-red-600">PULSE CHECK</span>
            </h1>
            
            <div className="max-w-3xl mx-auto">
              <p className={`text-[10px] font-mono uppercase tracking-[0.4em] mb-6 font-bold italic ${!selectedLens ? 'animate-pulse text-red-600' : 'text-slate-500'}`}>
                {selectedLens ? 'NODE_AUTHORIZED' : 'Select Active Operational Node'}
              </p>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  {[{ id: "EXECUTIVE", label: "EXECUTIVE", desc: "ROI / Fiduciary" }, { id: "OPERATIONAL", label: "OPERATIONAL", desc: "Value / Process" }, { id: "TECHNICAL", label: "TECHNICAL", desc: "Logic / Architecture" }].map((node) => (
                      <button key={node.id} onClick={() => setSelectedLens(node.id)} className={`p-6 border-2 flex flex-col items-center gap-2 transition-all ${selectedLens === node.id ? 'bg-red-600 border-red-600 text-white' : 'bg-slate-950 border-slate-900 text-slate-500 hover:border-slate-700'}`}>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
              {sectors.map((s) => (
                <button key={s.id} disabled={!selectedLens} onClick={() => { setSector(s.id); setStep("intake"); }} className="p-8 bg-slate-950/50 border-2 border-slate-900 hover:border-red-600 transition-all text-left flex flex-col justify-between h-48 group disabled:opacity-20">
        ...
        {step === 'intake' && (
          <motion.div key="intake" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-12 text-center">
            <h2 className="text-[40px] md:text-5xl font-black uppercase italic tracking-tighter text-white">PROTOCOL <span className="text-red-600">REGISTRATION</span></h2>
            <div className="bg-slate-950/30 border border-slate-900 p-6 md:p-12 max-w-4xl mx-auto space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-left">
                <input placeholder="OPERATOR_NAME" value={operatorName} onChange={(e) => setOperatorName(e.target.value)} className="bg-slate-950 border border-slate-800 p-6 text-white w-full uppercase font-mono text-sm" />
                <input placeholder="ORGANIZATION" value={entityName} onChange={(e) => setEntityName(e.target.value)} className="bg-slate-950 border border-slate-800 p-6 text-white w-full uppercase font-mono text-sm" />
                <input placeholder="SECURE_EMAIL" value={email} onChange={(e) => setEmail(e.target.value)} className="bg-slate-950 border border-slate-800 p-6 text-white w-full uppercase font-mono text-sm" />
                <input placeholder="CONFIRM_EMAIL" value={confirmEmail} onChange={(e) => setConfirmEmail(e.target.value)} className="bg-slate-950 border border-slate-800 p-6 text-white w-full uppercase font-mono text-sm" />
              </div>
        ...
        {step === 'audit' && (
          <motion.div key="audit" className="space-y-12 text-left">
            ...
            <h2 className="text-3xl md:text-6xl font-black italic uppercase text-white leading-tight min-h-[160px] tracking-tighter">{LOCAL_QUESTIONS[currentDimension]?.text}</h2>
            <div className="grid grid-cols-1 gap-4 mt-16">
              {LOCAL_QUESTIONS[currentDimension]?.options.map((opt, i) => (
                <button key={i} className="py-6 md:py-10 px-6 md:px-12 border-2 border-slate-800 bg-slate-950/20 hover:border-red-600 transition-all text-left uppercase font-black text-slate-400 hover:text-white flex justify-between items-center group" 
                  onClick={async () => {
                    ...
                    else { setIsLoading(true); const auditId = await logToDatabase(getLiveMetrics()); if (auditId) window.location.href = `/results/${auditId}`; }
                  }}>
