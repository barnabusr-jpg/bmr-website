...
// 🛡️ NOTE: We are using a local fallback for the loader to prevent branch-sync crashes
...
export default function ConsolidatedDiagnostic() {
  ...
  const logToDatabase = async (finalMetrics: any) => {
    try {
      ...
      // 2. Audit Initialization (Crucial: Using your exact column names)
      ...
      <AnimatePresence mode="wait">
        {isLoading && (
          <motion.div 
            ...
            animate={{ opacity: 1 }} 
            exit={{ opacity: 0 }}
            className="fixed inset-0 bg-black/90 z-[9999] flex flex-col items-center justify-center text-red-600"
          >
            <Activity className="animate-spin mb-4" size={48} />
            <p className="font-black italic uppercase tracking-[0.4em]">Synchronizing_Logic...</p>
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <AnimatePresence mode="wait">
        ...
        {step === 'intake' && (
          <motion.div key="intake" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-12 text-center">
            ...
            <div className="bg-slate-950/30 border border-slate-900 p-12 max-w-4xl mx-auto space-y-6 text-left">
              <div className="grid grid-cols-2 gap-6">
                <div className="space-y-2">
                  <label className="text-[9px] font-mono text-slate-500 uppercase tracking-widest">Operator_Identity</label>
                  <input placeholder="NAME" value={operatorName} onChange={(e) => setOperatorName(e.target.value)} className="bg-slate-950 border border-slate-800 p-6 text-white w-full uppercase font-mono outline-none focus:border-red-600" />
                </div>
                <div className="space-y-2">
                  <label className="text-[9px] font-mono text-slate-500 uppercase tracking-widest">Entity_Signal</label>
                  <input placeholder="ORGANIZATION" value={entityName} onChange={(e) => setEntityName(e.target.value)} className="bg-slate-950 border border-slate-800 p-6 text-white w-full uppercase font-mono outline-none focus:border-red-600" />
                </div>
                <div className="space-y-2">
                  <label className="text-[9px] font-mono text-slate-500 uppercase tracking-widest">Intel_Channel</label>
                  <input placeholder="EMAIL" value={email} onChange={(e) => setEmail(e.target.value)} className="bg-slate-950 border border-slate-800 p-6 text-white w-full uppercase font-mono outline-none focus:border-red-600" />
                </div>
                <div className="space-y-2">
                  <label className="text-[9px] font-mono text-slate-500 uppercase tracking-widest">Verify_Channel</label>
                  <input placeholder="CONFIRM_EMAIL" value={confirmEmail} onChange={(e) => setConfirmEmail(e.target.value)} className="bg-slate-950 border border-slate-800 p-6 text-white w-full uppercase font-mono outline-none focus:border-red-600" />
                </div>
              </div>
              <button disabled={!operatorName || email !== confirmEmail || !email} onClick={() => setStep("audit")} className="w-full py-8 font-black uppercase italic bg-red-600 text-white disabled:opacity-20 text-xl tracking-[0.2em] hover:bg-white hover:text-red-600 transition-all">Initialize Observation</button>
            </div>
          </motion.div>
        )}
