...
export default function ConsolidatedDiagnostic() {
  ...
  const logToDatabase = async (finalMetrics: any) => {
    // SINGLE FIX: Lock values here to prevent state-wipe linkage errors
    const finalOrg = entityName.trim().toUpperCase();
    const finalEmail = email.trim().toLowerCase();
    const finalName = operatorName.trim().toUpperCase();

    try {
      const { data: entityData } = await supabase.from('entities').upsert({ name: finalOrg }, { onConflict: 'name' }).select().single();
      const { data: operatorData } = await supabase.from('operators').upsert({ email: finalEmail, full_name: finalName, entity_id: entityData?.id }, { onConflict: 'email' }).select().single();
      
      // UPSERT on org_name ensures one clean row in the ledger
  ...
  // VISUAL FIX: Restored contrast to input boxes
  const inputClass = "bg-slate-900 border-2 border-slate-700 p-6 text-white w-full uppercase outline-none focus:border-red-600 font-mono text-xl placeholder:text-slate-700";
          ...
          <motion.div key="intake" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-12 text-center">
            ...
            <div className="bg-slate-950/30 border border-slate-900 p-12 max-w-4xl mx-auto space-y-6 shadow-2xl">
              <div className="grid grid-cols-2 gap-6">
                <input placeholder="OPERATOR_NAME" value={operatorName} onChange={(e) => setOperatorName(e.target.value)} className={inputClass} />
                <input placeholder="ORGANIZATION" value={entityName} onChange={(e) => setEntityName(e.target.value)} className={inputClass} />
                <input placeholder="SECURE_EMAIL" value={email} onChange={(e) => setEmail(e.target.value)} className={inputClass} />
                <input placeholder="CONFIRM_EMAIL" value={confirmEmail} onChange={(e) => setConfirmEmail(e.target.value)} className={inputClass} />
              </div>
          ...
          <motion.div key="verdict" className="py-10 text-center">
            <ForensicResultCard 
              result={{
                ...
                status: 'ACTIVE_SYNTHESIS',
                protocol: 'LEAD_NODE_VERIFIED'
              }} 
              lens={selectedLens} 
            />
            <div className="mt-12 text-slate-500 font-mono text-xs uppercase tracking-[0.3em] animate-pulse">
              SIGNAL_CAPTURED: Monitoring Admin Command Center for peer triangulation...
            </div>
          </motion.div>
        )}
      </AnimatePresence>
