...
export default class DossierEngine {
  static async generate(result: any, lens: string): Promise<void> {
    ...
    // --- NEW: Human-Friendly Label Mapping ---
    const LENS_MAP: Record<string, string> = {
      MGR: 'MANAGERIAL_STRATUM',
      TEC: 'TECHNICAL_CORE',
      EXE: 'EXECUTIVE_COMMAND'
    };
    const displayLens = LENS_MAP[lens.toUpperCase()] || lens.toUpperCase();
    ...
    doc.setFontSize(7);
    doc.text(`REF_ID: ${refId} // ORIGIN: ${displayLens} // ${ts}`, 20, 38);
    ...
    // Calculate how 'shrunken' the triangle is based on friction
    ...
    doc.circle(p1.x, p1.y, 1, 'F'); 
    doc.circle(p2.x, p2.y, 1, 'F'); 
    doc.circle(p3.x, p3.y, 1, 'F');

    // Label Nodes
    doc.setFontSize(8); 
    doc.text("NODE_01: FIDUCIARY", cx-10, cy-r-5);
    doc.text("NODE_02: OPERATIONAL", cx-(r*0.866)-10, cy+(r*0.5)+10);
    doc.text("NODE_03: INFRASTRUCTURE", cx+(r*0.866)-10, cy+(r*0.5)+10);
