...
// 📝 STRAT GRAPH MODEL CONTRACTS
export interface AnomalyMatrixNode {
  id: string;
  lens: "EXE" | "MGR" | "TEC" | "USR";
...
export interface CalculateMetricsParams {
  companyName: string;
  responses: Record<string, Record<string, string>> | any;
  sector: SectorType;
}
...
export const FORENSIC_MATRIX: AnomalyMatrixNode[] = [
  // --- T1: INDEMNITY ---
  ...
  { id: "TEC_07", lens: "TEC", triangulationId: "T7", weight: 15, text: "Are we using 'Grounding' datasets to constrain AI responses to our own data?", evidenceOptions: ["RISK_PIPELINE", "VECTOR_DB", "NONE"] },
...
// 🏎️ METRIC SCALING DECAY CONSTANTS
const RESPONSE_PENALTIES: Record<string, number> = {
  A: 0.00, // Nominal Operations Integrity
  B: 0.25, // Fragmented Pipeline Drift
  C: 0.65, // Active Capital Leakage
  D: 1.00  // Critical System Collapse Threshold
};
...
export function calculateForensicMetrics(
  ...
  // 1. Establish Sector Multiplier Enforcements
  ...
  // 2. Perform Out-of-Band Vector Extraction Logic Loops
  let totalTrackedQuestions = 0;
  let accumulatedPenaltyScore = 0;
  let criticalFailureCount = 0;

  if (responses && typeof responses === 'object') {
    Object.keys(responses).forEach((roleKey) => {
      const roleAnswers = responses[roleKey];
      if (roleAnswers && typeof roleAnswers === 'object') {
        Object.keys(roleAnswers).forEach((qId) => {
          const selectedChoice = String(roleAnswers[qId]).toUpperCase();
          const penaltyWeight = RESPONSE_PENALTIES[selectedChoice] ?? 0;
          
          accumulatedPenaltyScore += penaltyWeight;
  ...
  // 3. Apply Calibration Defaults if Matrix Is Initializing or Empty
  const totalMacroDrift = totalTrackedQuestions > 0 ? (accumulatedPenaltyScore / totalTrackedQuestions) : 0.40;

  // 4. Compound Financial Leakage Algorithms
  const complianceScoreBase = 100 * (1 - totalMacroDrift);
  ...
  return {
    companyName,
    multiplier,
    complianceScore: Math.round(Math.max(0, Math.min(100, complianceScoreBase))),
